        ...
        try {
            final InputStream inputStream =
                    Minecraft.getMinecraft()
                            .getResourceManager()
                            .getResource(new ResourceLocation(resourcePath))
                            .getInputStream();
    ...
    private void handleSprites(
        ...
        if(drawString) {
            final BufferedImage image =
                    new BufferedImage(fontData.width, fontData.height, BufferedImage.TYPE_INT_ARGB);
            ...
            final ByteBuffer buffer =
                    ByteBuffer.allocateDirect(pixels.length * 4).order(ByteOrder.nativeOrder());
            ...
            while(index < fontData.chars.length) {
                ...
                final Rectangle2D dimensions =
                        fontMetrics.getStringBounds(String.valueOf(character), graphics2D);
                ...
                if(charData.height > height) height = charData.height;
                if(charData.height > fontHeight) fontHeight = charData.height;
                charData.x = x;
                charData.y = y;
