  ...
  private void checkFall(World worldIn, BlockPos pos) {
    if (BlockFalling.canFallInto(worldIn, pos.down()) && pos.getY() >= 0) {
      ...
      if (!BlockFalling.fallInstantly
          && worldIn.isAreaLoaded(pos.add(-i, -i, -i), pos.add(i, i, i))) {
        ...
        for (blockpos = pos;
            BlockFalling.canFallInto(worldIn, blockpos) && blockpos.getY() > 0;
            ...
            for (int j = 0; j < 128; ++j) {
              ...
              double d1 =
                  (double) blockpos.getX()
                      ...
                      + (worldIn.rand.nextDouble() - 0.5D) * 1.0D
                      + 0.5D;
              double d2 =
                  (double) blockpos.getY()
                      + (double) (pos.getY() - blockpos.getY()) * d0
                      + worldIn.rand.nextDouble() * 1.0D
                      - 0.5D;
              double d3 =
