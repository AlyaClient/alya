...
public class BlockDirt extends Block {
  public static final PropertyEnum<BlockDirt.DirtType> VARIANT =
      PropertyEnum.<BlockDirt.DirtType>create("variant", BlockDirt.DirtType.class);
  ...
  public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
    if (state.getValue(VARIANT) == BlockDirt.DirtType.PODZOL) {
      ...
      state =
          state.withProperty(
              SNOWY, Boolean.valueOf(block == Blocks.snow || block == Blocks.snow_layer));
    }
  ...
  public int getDamageValue(World worldIn, BlockPos pos) {
    ...
    return iblockstate.getBlock() != this
        ? 0
        : ((BlockDirt.DirtType) iblockstate.getValue(VARIANT)).getMetadata();
  }
  ...
  public static enum DirtType implements IStringSerializable {
    ...
    private static final BlockDirt.DirtType[] METADATA_LOOKUP =
        new BlockDirt.DirtType[values().length];
    ...
    private DirtType(
        int p_i46397_3_, String p_i46397_4_, String p_i46397_5_, MapColor p_i46397_6_) {
