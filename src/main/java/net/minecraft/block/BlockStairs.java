...
public class BlockStairs extends Block {
  ...
  public static final PropertyEnum<BlockStairs.EnumHalf> HALF =
      PropertyEnum.<BlockStairs.EnumHalf>create("half", BlockStairs.EnumHalf.class);
  public static final PropertyEnum<BlockStairs.EnumShape> SHAPE =
      PropertyEnum.<BlockStairs.EnumShape>create("shape", BlockStairs.EnumShape.class);
  private static final int[][] field_150150_a =
      new int[][] {{4, 5}, {5, 7}, {6, 7}, {4, 6}, {0, 1}, {1, 3}, {2, 3}, {0, 2}};
  ...
  /**
   * Check whether there is a stair block at the given position and it has the same properties as
   * the given BlockState
   */
  public static boolean isSameStair(IBlockAccess worldIn, BlockPos pos, IBlockState state) {
    IBlockState iblockstate = worldIn.getBlockState(pos);
    Block block = iblockstate.getBlock();
    return isBlockStairs(block)
    ...
    if (enumfacing == EnumFacing.EAST) {
      ...
      if (isBlockStairs(block) && blockstairs$enumhalf == iblockstate1.getValue(HALF)) {
        ...
        if (enumfacing1 == EnumFacing.NORTH
            && !isSameStair(blockAccess, pos.south(), iblockstate)) {
          return flag ? 1 : 2;
        }

        if (enumfacing1 == EnumFacing.SOUTH
            && !isSameStair(blockAccess, pos.north(), iblockstate)) {
          return flag ? 2 : 1;
        }
      }
      ...
      if (isBlockStairs(block2) && blockstairs$enumhalf == iblockstate3.getValue(HALF)) {
        ...
        if (enumfacing3 == EnumFacing.WEST && !isSameStair(blockAccess, pos.east(), iblockstate)) {
          ...
          flag1 = false;
            ...
            && !isSameStair(blockAccess, pos.west(), iblockstate)) {
          f2 = 0.5F;
          flag1 = false;
        }
      }
      ...
      if (isBlockStairs(block2) && blockstairs$enumhalf == iblockstate3.getValue(HALF)) {
        ...
        if (enumfacing3 == EnumFacing.WEST && !isSameStair(blockAccess, pos.west(), iblockstate)) {
          flag1 = true;
            ...
            && !isSameStair(blockAccess, pos.east(), iblockstate)) {
          f2 = 0.5F;
          f3 = 1.0F;
          flag1 = true;
        }
  ...
  public boolean onBlockActivated(
      ...
      float hitY,
      float hitZ) {
    return this.modelBlock.onBlockActivated(
        worldIn, pos, this.modelState, playerIn, EnumFacing.DOWN, 0.0F, 0.0F, 0.0F);
  }
  ...
  public IBlockState getStateFromMeta(int meta) {
    IBlockState iblockstate =
        this.getDefaultState()
            .withProperty(
                HALF, (meta & 4) > 0 ? BlockStairs.EnumHalf.TOP : BlockStairs.EnumHalf.BOTTOM);
