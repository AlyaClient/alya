...
public class BlockPattern {
  ...
  private BlockPattern.PatternHelper checkPatternAt(
      BlockPos pos,
      EnumFacing finger,
      EnumFacing thumb,
    ...
    for (int i = 0; i < this.palmLength; ++i) {
      for (int j = 0; j < this.thumbLength; ++j) {
        for (int k = 0; k < this.fingerLength; ++k) {
          if (!this.blockMatches[k][j][i].apply(
              lcache.getUnchecked(translateOffset(pos, finger, thumb, i, j, k)))) {
            return null;
          }
        }
      }
    ...
    return new BlockPattern.PatternHelper(
        pos, finger, thumb, lcache, this.palmLength, this.thumbLength, this.fingerLength);
  }

  /**
   * Calculates whether the given world position matches the pattern. Warning, fairly heavy
   * function. @return a BlockPattern.PatternHelper if found, null otherwise.
   */
        ...
        for (EnumFacing enumfacing1 : EnumFacing.values()) {
          if (enumfacing1 != enumfacing && enumfacing1 != enumfacing.getOpposite()) {
            BlockPattern.PatternHelper blockpattern$patternhelper =
                this.checkPatternAt(blockpos, enumfacing, enumfacing1, loadingcache);
  ...
  public static LoadingCache<BlockPos, BlockWorldState> func_181627_a(
      World p_181627_0_, boolean p_181627_1_) {
   ...
   * follows the right-hand rule for cross products (finger, thumb, palm) @return A new BlockPos
   * offset in the facing directions
   *
   * @param pos The original block position
   * @param finger Finger direction
   * @param thumb Thumb direction
   * @param palmOffset An amount to offset in the palm direction. Palm is the direction of the
   *     result of the cross- product between finger and thumb
   ...
   */
  protected static BlockPos translateOffset(
      BlockPos pos,
      EnumFacing finger,
      EnumFacing thumb,
      int palmOffset,
    ...
    if (finger != thumb && finger != thumb.getOpposite()) {
      Vec3i vec3i =
          new Vec3i(finger.getFrontOffsetX(), finger.getFrontOffsetY(), finger.getFrontOffsetZ());
      Vec3i vec3i1 =
