...
public abstract class BlockLiquid extends Block {
  ...
  /**
   * Returns the percentage of the liquid block that is air, based on the given flow decay of the
   * liquid
   */
  ...
  public AxisAlignedBB getCollisionBoundingBox(World worldIn, BlockPos pos, IBlockState state) {
    if (dev.thoq.lua.api.LuaMinecraftApi.jesusActive) {
      ...
      if (j < 0) {
        if (!worldIn.getBlockState(blockpos).getBlock().getMaterial().blocksMovement()) {
          ...
          if (j >= 0) {
            ...
            vec3 =
                vec3.addVector(
                    (double) ((blockpos.getX() - pos.getX()) * k),
        ...
        vec3 =
            vec3.addVector(
                (double) ((blockpos.getX() - pos.getX()) * l),
                (double) ((blockpos.getY() - pos.getY()) * l),
                (double) ((blockpos.getZ() - pos.getZ()) * l));
      }
    }
  ...
  public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
    ...
    if (rand.nextInt(10) == 0 && World.doesBlockHaveSolidTopSurface(worldIn, pos.down())) {
      ...
      if (!material.blocksMovement() && !material.isLiquid()) {
        ...
        if (this.blockMaterial == Material.water) {
          worldIn.spawnParticle(
              EnumParticleTypes.DRIP_WATER, d3, d5, d7, 0.0D, 0.0D, 0.0D, new int[0]);
        } else {
          worldIn.spawnParticle(
              EnumParticleTypes.DRIP_LAVA, d3, d5, d7, 0.0D, 0.0D, 0.0D, new int[0]);
        }
      }
    }
  }
  ...
  public static double getFlowDirection(IBlockAccess worldIn, BlockPos pos, Material materialIn) {
    ...
    return vec3.xCoord == 0.0D && vec3.zCoord == 0.0D
        ? -1000.0D
