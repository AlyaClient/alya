...
public class BlockFire extends Block {
  ...
  public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
    ...
    if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down())
      ...
      return state
          .withProperty(NORTH, Boolean.valueOf(this.canCatchFire(worldIn, pos.north())))
  ...
  protected BlockFire() {
    ...
    this.setDefaultState(
        this.blockState
            ...
            .withProperty(FLIP, Boolean.valueOf(false))
            .withProperty(ALT, Boolean.valueOf(false))
    ...
    if (random.nextInt(chance) < i) {
      ...
      if (iblockstate.getBlock() == Blocks.tnt) {
        Blocks.tnt.onBlockDestroyedByPlayer(
            worldIn, pos, iblockstate.withProperty(BlockTNT.EXPLODE, Boolean.valueOf(true)));
      }
    }
  }
  ...
  private int getNeighborEncouragement(World worldIn, BlockPos pos) {
    if (!worldIn.isAirBlock(pos)) {
      ...
      for (EnumFacing enumfacing : EnumFacing.values()) {
        i =
            Math.max(
                this.getEncouragement(worldIn.getBlockState(pos.offset(enumfacing)).getBlock()), i);
      }
  ...
  public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
    return World.doesBlockHaveSolidTopSurface(worldIn, pos.down())
        || this.canNeighborCatchFire(worldIn, pos);
  }
    ...
    if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down())
      ...
      if (Blocks.fire.canCatchFire(worldIn, pos.west())) {
        for (int j = 0; j < 2; ++j) {
          ...
          worldIn.spawnParticle(
              EnumParticleTypes.SMOKE_LARGE, d3, d8, d13, 0.0D, 0.0D, 0.0D, new int[0]);
        }
      }
      ...
      if (Blocks.fire.canCatchFire(worldIn, pos.east())) {
        for (int k = 0; k < 2; ++k) {
          ...
          worldIn.spawnParticle(
              EnumParticleTypes.SMOKE_LARGE, d4, d9, d14, 0.0D, 0.0D, 0.0D, new int[0]);
        }
      }
      ...
      if (Blocks.fire.canCatchFire(worldIn, pos.north())) {
        for (int l = 0; l < 2; ++l) {
          ...
          worldIn.spawnParticle(
              EnumParticleTypes.SMOKE_LARGE, d5, d10, d15, 0.0D, 0.0D, 0.0D, new int[0]);
        }
      }
      ...
      if (Blocks.fire.canCatchFire(worldIn, pos.south())) {
        for (int i1 = 0; i1 < 2; ++i1) {
          ...
          worldIn.spawnParticle(
              EnumParticleTypes.SMOKE_LARGE, d6, d11, d16, 0.0D, 0.0D, 0.0D, new int[0]);
        }
      }
      ...
      if (Blocks.fire.canCatchFire(worldIn, pos.up())) {
        for (int j1 = 0; j1 < 2; ++j1) {
          ...
          worldIn.spawnParticle(
              EnumParticleTypes.SMOKE_LARGE, d7, d12, d17, 0.0D, 0.0D, 0.0D, new int[0]);
        }
      }
    } else {
      for (int i = 0; i < 3; ++i) {
        ...
        worldIn.spawnParticle(
            EnumParticleTypes.SMOKE_LARGE, d0, d1, d2, 0.0D, 0.0D, 0.0D, new int[0]);
      }
    }
  }
