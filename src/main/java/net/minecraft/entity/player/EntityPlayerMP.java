...
public class EntityPlayerMP extends EntityPlayer implements ICrafting {
  ...
  /**
   * the total health of the player, includes actual health and absorption health. Updated every
   * tick.
   */
  ...
  /**
   * set to true when player is moving quantity of items from one inventory to another(crafting) but
   * item in either slot is not changed
   */
  ...
  /**
   * Set when a player beats the ender dragon, used to respawn the player at the spawn point while
   * retaining inventory and XP
   */
  ...
  public void sendEnterCombat() {
    ...
    this.playerNetServerHandler.sendPacket(
        new S42PacketCombatEvent(this.getCombatTracker(), S42PacketCombatEvent.Event.ENTER_COMBAT));
  }
  ...
  public void sendEndCombat() {
    super.sendEndCombat();
    this.playerNetServerHandler.sendPacket(
        new S42PacketCombatEvent(this.getCombatTracker(), S42PacketCombatEvent.Event.END_COMBAT));
  }
  ...
  public void onUpdate() {
    ...
    if (!this.loadedChunks.isEmpty()) {
      ...
      if (!list.isEmpty()) {
        if (list.size() == 1) {
          this.playerNetServerHandler.sendPacket(
              new S21PacketChunkData((Chunk) list.get(0), true, 65535));
        } else {
    ...
    try {
      ...
      if (this.getHealth() + this.getAbsorptionAmount() != this.combinedHealth) {
        ...
        for (ScoreObjective scoreobjective :
            this.getWorldScoreboard().getObjectivesFromCriteria(IScoreObjectiveCriteria.health)) {
  ...
  /**
   * Updates all biomes that have been explored by this player and triggers Adventuring Time if
   * player qualifies.
   */
  protected void updateBiomesExplored() {
    ...
    JsonSerializableSet jsonserializableset =
        (JsonSerializableSet) this.getStatFile().func_150870_b(AchievementList.exploreAllBiomes);
      ...
      if (team != null && team.getDeathMessageVisibility() != Team.EnumVisible.ALWAYS) {
        if (team.getDeathMessageVisibility() == Team.EnumVisible.HIDE_FOR_OTHER_TEAMS) {
          this.mcServer
              ...
              .sendMessageToAllTeamMembers(this, this.getCombatTracker().getDeathMessage());
        } else if (team.getDeathMessageVisibility() == Team.EnumVisible.HIDE_FOR_OWN_TEAM) {
          this.mcServer
              .getConfigurationManager()
              .sendMessageToTeamOrEvryPlayer(this, this.getCombatTracker().getDeathMessage());
        }
      } else {
        this.mcServer
            .getConfigurationManager()
            .sendChatMsg(this.getCombatTracker().getDeathMessage());
      }
    }
    ...
    if (entitylivingbase != null) {
      EntityList.EntityEggInfo entitylist$entityegginfo =
          (EntityList.EntityEggInfo)
              EntityList.entityEggs.get(Integer.valueOf(EntityList.getEntityID(entitylivingbase)));
  ...
  public boolean attackEntityFrom(DamageSource source, float amount) {
    if (this.isEntityInvulnerable(source)) {
      ...
      boolean flag =
          this.mcServer.isDedicatedServer()
              && this.canPlayersAttack()
  ...
  public void displayGui(IInteractionObject guiOwner) {
    ...
    this.playerNetServerHandler.sendPacket(
        new S2DPacketOpenWindow(
            this.currentWindowId, guiOwner.getGuiID(), guiOwner.getDisplayName()));
  ...
  public void displayGUIChest(IInventory chestInventory) {
        ...
        this.playerNetServerHandler.sendPacket(
            new S02PacketChat(
                new ChatComponentTranslation(
                    "container.isLocked", new Object[] {chestInventory.getDisplayName()}),
                (byte) 2));
        this.playerNetServerHandler.sendPacket(
            new S29PacketSoundEffect(
                "random.door_close", this.posX, this.posY, this.posZ, 1.0F, 1.0F));
        return;
      }
    }
    ...
    if (chestInventory instanceof IInteractionObject) {
      ...
      this.openContainer =
          ((IInteractionObject) chestInventory).createContainer(this.inventory, this);
    } else {
      this.playerNetServerHandler.sendPacket(
          new S2DPacketOpenWindow(
              this.currentWindowId,
  ...
  public void displayGUIHorse(EntityHorse horse, IInventory horseInventory) {
    ...
    this.playerNetServerHandler.sendPacket(
        new S2DPacketOpenWindow(
            this.currentWindowId,
            "EntityHorse",
            horseInventory.getDisplayName(),
            horseInventory.getSizeInventory(),
            horse.getEntityId()));
  ...
  public void displayGUIBook(ItemStack bookStack) {
    ...
    if (item == Items.written_book) {
      this.playerNetServerHandler.sendPacket(
          new S3FPacketCustomPayload("MC|BOpen", new PacketBuffer(Unpooled.buffer())));
    }
  }
  ...
  public void updateCraftingInventory(Container containerToSend, List<ItemStack> itemsList) {
    this.playerNetServerHandler.sendPacket(
        new S30PacketWindowItems(containerToSend.windowId, itemsList));
    this.playerNetServerHandler.sendPacket(
        new S2FPacketSetSlot(-1, -1, this.inventory.getItemStack()));
  }

  /**
   * Sends two ints to the client-side Container. Used for furnace burning time, smelting progress,
   * brewing progress, and enchanting level. Normally the first int identifies which variable to
   * update, and the second contains the new value. Both are truncated to shorts in non-local SMP.
   *
  ...
  public void sendProgressBarUpdate(Container containerIn, int varToUpdate, int newValue) {
    this.playerNetServerHandler.sendPacket(
        new S31PacketWindowProperty(containerIn.windowId, varToUpdate, newValue));
  }

  public void func_175173_a(Container p_175173_1_, IInventory p_175173_2_) {
    for (int i = 0; i < p_175173_2_.getFieldCount(); ++i) {
      this.playerNetServerHandler.sendPacket(
          new S31PacketWindowProperty(p_175173_1_.windowId, i, p_175173_2_.getField(i)));
    }
  }
  ...
  public void setEntityActionState(
      float p_110430_1_, float p_110430_2_, boolean p_110430_3_, boolean sneaking) {
  ...
  public void addStat(StatBase stat, int amount) {
    if (stat != null) {
      ...
      for (ScoreObjective scoreobjective :
          this.getWorldScoreboard().getObjectivesFromCriteria(stat.func_150952_k())) {
        this.getWorldScoreboard()
            .getValueFromObjective(this.getCommandSenderName(), scoreobjective)
            .increseScore(amount);
      }
  ...
  public void func_175145_a(StatBase p_175145_1_) {
    if (p_175145_1_ != null) {
      ...
      for (ScoreObjective scoreobjective :
          this.getWorldScoreboard().getObjectivesFromCriteria(p_175145_1_.func_150952_k())) {
        this.getWorldScoreboard()
            .getValueFromObjective(this.getCommandSenderName(), scoreobjective)
            .setScorePoints(0);
      }
  ...
  /**
   * this function is called when a players inventory is sent to him, lastHealth is updated on any
   * dimension transitions, then reset.
   */
  ...
  /**
   * Copies the values from the given player into this player if boolean par2 is true. Always clones
  ...
  protected void onFinishedPotionEffect(PotionEffect p_70688_1_) {
    ...
    this.playerNetServerHandler.sendPacket(
        new S1EPacketRemoveEntityEffect(this.getEntityId(), p_70688_1_));
  }
  ...
  public void onCriticalHit(Entity entityHit) {
    this.getServerForPlayer()
        .getEntityTracker()
        .func_151248_b(this, new S0BPacketAnimation(entityHit, 4));
  }

  public void onEnchantmentCritical(Entity entityHit) {
    this.getServerForPlayer()
        .getEntityTracker()
        .func_151248_b(this, new S0BPacketAnimation(entityHit, 5));
  }
  ...
  /**
   * Returns {@code true} if the CommandSender is allowed to execute the command, {@code false} if
   * not
   *
  ...
  public boolean canCommandSenderUseCommand(int permLevel, String commandName) {
    if ("seed".equals(commandName) && !this.mcServer.isDedicatedServer()) {
      return true;
    } else if (!"tell".equals(commandName)
      ...
      if (this.mcServer.getConfigurationManager().canSendCommands(this.getGameProfile())) {
        ...
        return userlistopsentry != null
            ? userlistopsentry.getPermissionLevel() >= permLevel
  ...
  public void removeEntity(Entity p_152339_1_) {
    if (p_152339_1_ instanceof EntityPlayer) {
      this.playerNetServerHandler.sendPacket(
          new S13PacketDestroyEntities(new int[] {p_152339_1_.getEntityId()}));
    } else {
  ...
  /**
   * Clears potion metadata values if the entity has no potion effects. Otherwise, updates potion
   * effect color, ambience, and invisibility metadata values
   */
  ...
  public void setSpectatingEntity(Entity entityToSpectate) {
    ...
    if (entity != this.spectatingEntity) {
      ...
      this.setPositionAndUpdate(
          this.spectatingEntity.posX, this.spectatingEntity.posY, this.spectatingEntity.posZ);
    }
  }
  ...
  /**
   * Returns null which indicates the tab list should just display the player's name, return a
   * different value to display the specified text instead of the player's name
   */
