...
public abstract class EntityPlayer extends EntityLivingBase {
  ...
  /**
   * Used to tell if the player pressed jump twice. If this is at 0 and it's pressed (And they are
   * allowed to fly, as defined in the player's movementInput) it sets this to 7. If it's pressed
   * and it's greater than 0 enable fly.
   */
  ...
  /**
   * The total amount of experience the player has. This also includes the amount of experience
  ...
  /**
   * This is the item that is in use when the player is holding down the useItemButton (e.g., bow,
   * food, sword)
   */
  ...
  /**
   * An instance of a fishing rod's hook. If this isn't null, the icon image of the fishing rod is
   * slightly different
   */
  ...
  /**
   * Checks if the entity is currently using an item (e.g., bow, food, sword) by holding down the
   * useItemButton
   */
      ...
      if (this.ridingEntity != null && !this.ridingEntity.isDead) {
        axisalignedbb =
            this.getEntityBoundingBox()
                .union(this.ridingEntity.getEntityBoundingBox())
  ...
  public void addToPlayerScore(Entity entityIn, int amount) {
    ...
    Collection<ScoreObjective> collection =
        this.getWorldScoreboard().getObjectivesFromCriteria(IScoreObjectiveCriteria.totalKillCount);

    if (entityIn instanceof EntityPlayer) {
      this.triggerAchievement(StatList.playerKillsStat);
      collection.addAll(
          this.getWorldScoreboard()
              .getObjectivesFromCriteria(IScoreObjectiveCriteria.playerKillCount));
  ...
  private Collection<ScoreObjective> func_175137_e(Entity p_175137_1_) {
    ScorePlayerTeam scoreplayerteam =
        this.getWorldScoreboard().getPlayersTeam(this.getCommandSenderName());
    ...
    if (scoreplayerteam != null) {
      ...
      if (i >= 0 && i < IScoreObjectiveCriteria.field_178793_i.length) {
        for (ScoreObjective scoreobjective :
          ...
          Score score =
              this.getWorldScoreboard()
                  .getValueFromObjective(p_175137_1_.getCommandSenderName(), scoreobjective);
          score.func_96648_a();
        }
      }
    }
    ...
    ScorePlayerTeam scoreplayerteam1 =
        this.getWorldScoreboard().getPlayersTeam(p_175137_1_.getCommandSenderName());
    ...
    if (scoreplayerteam1 != null) {
      ...
      if (j >= 0 && j < IScoreObjectiveCriteria.field_178792_h.length) {
        return this.getWorldScoreboard()
            .getObjectivesFromCriteria(IScoreObjectiveCriteria.field_178792_h[j]);
      }
    }
        ...
        entityitem.motionZ =
            (double)
                (MathHelper.cos(this.rotationYaw / 180.0F * (float) Math.PI)
                    ...
                    * f2);
        entityitem.motionY =
            (double) (-MathHelper.sin(this.rotationPitch / 180.0F * (float) Math.PI) * f2 + 0.1F);
  ...
  /**
   * Checks if the player has the ability to harvest a block (checks current inventory item for a
   * tool if necessary)
   */
  ...
  public void readEntityFromNBT(NBTTagCompound tagCompund) {
    ...
    if (tagCompund.hasKey("SpawnX", 99)
        && tagCompund.hasKey("SpawnY", 99)
        && tagCompund.hasKey("SpawnZ", 99)) {
      this.spawnChunk =
          new BlockPos(
              tagCompund.getInteger("SpawnX"),
              tagCompund.getInteger("SpawnY"),
  ...
  /**
   * When searching for vulnerable players, if a player is invisible, the return value of this is
   * the chance of seeing them anyway.
   */
        ...
        if (targetEntity instanceof EntityLivingBase) {
          f1 =
              EnchantmentHelper.func_152377_a(
                  this.getHeldItem(), ((EntityLivingBase) targetEntity).getCreatureAttribute());
        } else {
  ...
  public EntityPlayer.EnumStatus trySleep(BlockPos bedLocation) {
    if (!this.worldObj.isRemote) {
      ...
      double d1 = 5.0D;
      List<EntityMob> list =
          this.worldObj.<EntityMob>getEntitiesWithinAABB(
              EntityMob.class,
              new AxisAlignedBB(
    ...
    if (this.worldObj.isBlockLoaded(bedLocation)) {
      EnumFacing enumfacing =
          (EnumFacing) this.worldObj.getBlockState(bedLocation).getValue(BlockDirectional.FACING);
      float f = 0.5F;
      float f1 = 0.5F;
      ...
      this.setPosition(
          (double) ((float) bedLocation.getX() + f),
  ...
  public void wakeUpPlayer(boolean p_70999_1_, boolean updateWorldFlag, boolean setSpawn) {
    ...
    if (this.playerLocation != null && iblockstate.getBlock() == Blocks.bed) {
      this.worldObj.setBlockState(
          this.playerLocation,
          iblockstate.withProperty(BlockBed.OCCUPIED, Boolean.valueOf(false)),
          4);
   ...
   */
  public static BlockPos getBedSpawnLocation(
      World worldIn, BlockPos bedLocation, boolean forceSpawn) {
  ...
  public float getBedOrientationInDegrees() {
    if (this.playerLocation != null) {
      EnumFacing enumfacing =
          (EnumFacing)
              this.worldObj.getBlockState(this.playerLocation).getValue(BlockDirectional.FACING);
  ...
  public void moveEntityWithHeading(float strafe, float forward) {
    ...
    if (this.capabilities.isFlying && this.ridingEntity == null) {
      ...
      this.jumpMovementFactor =
          this.capabilities.getFlySpeed() * (float) (this.isSprinting() ? 2 : 1);
  ...
  public void addMovementStat(double p_71000_1_, double p_71000_3_, double p_71000_5_) {
    if (this.ridingEntity == null) {
      if (this.isInsideOfMaterial(Material.water)) {
        int i =
            Math.round(
                MathHelper.sqrt_double(
                        p_71000_1_ * p_71000_1_ + p_71000_3_ * p_71000_3_ + p_71000_5_ * p_71000_5_)
                    * 100.0F);
        ...
        int j =
            Math.round(
                MathHelper.sqrt_double(p_71000_1_ * p_71000_1_ + p_71000_5_ * p_71000_5_) * 100.0F);
  ...
  private void addMountedMovementStat(double p_71015_1_, double p_71015_3_, double p_71015_5_) {
    if (this.ridingEntity != null) {
      int i =
          Math.round(
              MathHelper.sqrt_double(
                      p_71015_1_ * p_71015_1_ + p_71015_3_ * p_71015_3_ + p_71015_5_ * p_71015_5_)
                  * 100.0F);
  ...
  public void onKillEntity(EntityLivingBase entityLivingIn) {
    ...
    EntityList.EntityEggInfo entitylist$entityegginfo =
        (EntityList.EntityEggInfo)
            EntityList.entityEggs.get(Integer.valueOf(EntityList.getEntityID(entityLivingIn)));
  ...
  public void addExperience(int amount) {
    ...
    for (this.experienceTotal += amount;
  ...
  /**
   * This method returns the cap amount of experience that the experience bar can hold. With each
   * level, the experience cap on the player's experience bar is raised by 10.
   */
  public int xpBarCap() {
  ...
  public boolean canPlayerEdit(
      BlockPos p_175151_1_, EnumFacing p_175151_2_, ItemStack p_175151_3_) {
  ...
  public void clonePlayer(EntityPlayer oldPlayer, boolean respawnFromEnd) {
    ...
    this.getDataWatcher()
        .updateObject(10, Byte.valueOf(oldPlayer.getDataWatcher().getWatchableObjectByte(10)));
  }
  ...
  public ItemStack getEquipmentInSlot(int slotIn) {
    return slotIn == 0
        ...
        : this.inventory.armorInventory[slotIn - 1];
  }
  ...
  public IChatComponent getDisplayName() {
    ...
    ichatcomponent
        ...
        .setChatClickEvent(
            new ClickEvent(
                ClickEvent.Action.SUGGEST_COMMAND, "/msg " + this.getCommandSenderName() + " "));
  ...
  public boolean isWearing(EnumPlayerModelParts p_175148_1_) {
    return (this.getDataWatcher().getWatchableObjectByte(10) & p_175148_1_.getPartMask())
        == p_175148_1_.getPartMask();
  }
    ...
    if (inventorySlot >= 0 && inventorySlot < this.inventory.mainInventory.length) {
      ...
      if (i >= 0 && i < this.inventory.armorInventory.length) {
        ...
        if (itemStackIn != null && itemStackIn.getItem() != null) {
          if (itemStackIn.getItem() instanceof ItemArmor) {
            if (EntityLiving.getArmorPosition(itemStackIn) != k) {
              ...
              || itemStackIn.getItem() != Items.skull
        ...
        this.inventory.setInventorySlotContents(
            i + this.inventory.mainInventory.length, itemStackIn);
        return true;
      } else {
  ...
  public static enum EnumChatVisibility {
    ...
    private static final EntityPlayer.EnumChatVisibility[] ID_LOOKUP =
        new EntityPlayer.EnumChatVisibility[values().length];
