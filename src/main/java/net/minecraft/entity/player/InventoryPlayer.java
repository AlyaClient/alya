...
public class InventoryPlayer implements IInventory {
  ...
  /**
   * Set true whenever the inventory changes. Nothing sets it false so you will have to write your
   * own code to check it and reset the value.
   */
  ...
  public ItemStack getCurrentItem() {
    return this.currentItem < 9 && this.currentItem >= 0
        ? this.mainInventory[this.currentItem]
        : null;
  }
  ...
  private int getInventorySlotContainItemAndDamage(Item itemIn, int p_146024_2_) {
    for (int i = 0; i < this.mainInventory.length; ++i) {
      if (this.mainInventory[i] != null
          && this.mainInventory[i].getItem() == itemIn
  ...
  public void setCurrentItem(
      Item itemIn, int p_146030_2_, boolean p_146030_3_, boolean p_146030_4_) {
    ItemStack itemstack = this.getCurrentItem();
    int i =
        p_146030_3_
            ? this.getInventorySlotContainItemAndDamage(itemIn, p_146030_2_)
   ...
   */
  public int clearMatchingItems(
      Item itemIn, int metadataIn, int removeCount, NBTTagCompound itemNBT) {
    int i = 0;
    ...
    for (int j = 0; j < this.mainInventory.length; ++j) {
      ...
      if (itemstack != null
          ...
          && (itemNBT == null
              || NBTUtil.func_181123_a(itemNBT, itemstack.getTagCompound(), true))) {
        int k =
            removeCount <= 0 ? itemstack.stackSize : Math.min(removeCount - i, itemstack.stackSize);
        i += k;
    ...
    for (int l = 0; l < this.armorInventory.length; ++l) {
      ...
      if (itemstack1 != null
          ...
          && (itemNBT == null
              || NBTUtil.func_181123_a(itemNBT, itemstack1.getTagCompound(), false))) {
        int j1 =
            removeCount <= 0
                ? itemstack1.stackSize
                : Math.min(removeCount - i, itemstack1.stackSize);
        i += j1;
    ...
    if (this.itemStack != null) {
      ...
      if (itemNBT != null
          && !NBTUtil.func_181123_a(itemNBT, this.itemStack.getTagCompound(), false)) {
        return i;
      }
      ...
      int i1 =
          removeCount <= 0
              ...
              : Math.min(removeCount - i, this.itemStack.stackSize);
      i += i1;
    ...
    if (j < 0) {
      ...
      if (this.mainInventory[j] == null) {
        ...
        if (itemStackIn.hasTagCompound()) {
          this.mainInventory[j].setTagCompound(
              (NBTTagCompound) itemStackIn.getTagCompound().copy());
        }
      }
  ...
  public void decrementAnimations() {
    for (int i = 0; i < this.mainInventory.length; ++i) {
      if (this.mainInventory[i] != null) {
        this.mainInventory[i].updateAnimation(
    ...
    if (itemStackIn != null && itemStackIn.stackSize != 0 && itemStackIn.getItem() != null) {
      try {
        ...
        CrashReport crashreport =
            CrashReport.makeCrashReport(throwable, "Adding item to inventory");
        CrashReportCategory crashreportcategory = crashreport.makeCategory("Item being added");
        crashreportcategory.addCrashSection(
            "Item ID", Integer.valueOf(Item.getIdFromItem(itemStackIn.getItem())));
        crashreportcategory.addCrashSection(
            "Item data", Integer.valueOf(itemStackIn.getMetadata()));
