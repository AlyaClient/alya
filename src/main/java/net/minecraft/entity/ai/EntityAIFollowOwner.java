...
public class EntityAIFollowOwner extends EntityAIBase {
  ...
  public EntityAIFollowOwner(
      EntityTameable thePetIn, double followSpeedIn, float minDistIn, float maxDistIn) {
  ...
  public boolean shouldExecute() {
    ...
    if (entitylivingbase == null) {
      ...
      return false;
    } else if (this.thePet.getDistanceSqToEntity(entitylivingbase)
        < (double) (this.minDist * this.minDist)) {
      return false;
    } else {
                ...
                for (int i1 = 0; i1 <= 4; ++i1) {
                  if ((l < 1 || i1 < 1 || l > 3 || i1 > 3)
                      && World.doesBlockHaveSolidTopSurface(
                          this.theWorld, new BlockPos(i + l, k - 1, j + i1))
