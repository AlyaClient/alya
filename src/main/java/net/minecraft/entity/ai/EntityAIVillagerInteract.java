...
public class EntityAIVillagerInteract extends EntityAIWatchClosest2 {
  ...
  public void startExecuting() {
    ...
    if (this.villager.canAbondonItems()
        && this.closestEntity instanceof EntityVillager
    ...
    if (this.interactionDelay > 0) {
      ...
      if (this.interactionDelay == 0) {
        ...
        for (int i = 0; i < inventorybasic.getSizeInventory(); ++i) {
          ...
          if (itemstack1 != null) {
            double d0 =
                this.villager.posY - 0.30000001192092896D + (double) this.villager.getEyeHeight();
            ...
            entityitem.motionZ =
                (double)
                    (MathHelper.cos(f1 / 180.0F * (float) Math.PI)
                        ...
                        * f);
            entityitem.motionY =
                (double) (-MathHelper.sin(f2 / 180.0F * (float) Math.PI) * f + 0.1F);
