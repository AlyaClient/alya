...
public class EntityAIMoveIndoors extends EntityAIBase {
  ...
  public boolean shouldExecute() {
    ...
    if ((!this.entityObj.worldObj.isDaytime()
            || this.entityObj.worldObj.isRaining()
      ...
      if (this.entityObj.getRNG().nextInt(50) != 0) {
        ...
        Village village =
            this.entityObj.worldObj.getVillageCollection().getNearestVillage(blockpos, 14);
    ...
    if (this.entityObj.getDistanceSq(blockpos) > 256.0D) {
      Vec3 vec3 =
          RandomPositionGenerator.findRandomTargetBlockTowards(
              this.entityObj, 14, 3, new Vec3((double) i + 0.5D, (double) j, (double) k + 0.5D));
