...
public class RandomPositionGenerator {
   ...
   * NEVER THREAD SAFE. MULTIPLE findTowards and findAway calls, will share this var
   */
  ...
  public static Vec3 findRandomTargetBlockTowards(
      EntityCreature entitycreatureIn, int xz, int y, Vec3 targetVec3) {
    staticVector =
        targetVec3.subtract(entitycreatureIn.posX, entitycreatureIn.posY, entitycreatureIn.posZ);
  ...
  public static Vec3 findRandomTargetBlockAwayFrom(
      EntityCreature entitycreatureIn, int xz, int y, Vec3 targetVec3) {
    staticVector =
        (new Vec3(entitycreatureIn.posX, entitycreatureIn.posY, entitycreatureIn.posZ))
            .subtract(targetVec3);
    return findRandomTargetBlock(entitycreatureIn, xz, y, staticVector);
  }

  /**
   * searches 10 blocks at random in a within par1(x,z) and par2 (y) distance, ignores those not in
   * the direction of par3Vec3, then points to the tile for which creature.getBlockPathWeight
  ...
  private static Vec3 findRandomTargetBlock(
    ...
    for (int j1 = 0; j1 < 10; ++j1) {
      ...
      if (targetVec3 == null
          || (double) l * targetVec3.xCoord + (double) i1 * targetVec3.zCoord >= 0.0D) {
