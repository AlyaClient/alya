...
public class DataWatcher {
  ...
  private final Map<Integer, DataWatcher.WatchableObject> watchedObjects =
      Maps.<Integer, DataWatcher.WatchableObject>newHashMap();
  ...
  public <T> void addObject(int id, T object) {
    ...
    if (integer == null) {
      ...
      throw new IllegalArgumentException(
          "Data value id is too big with " + id + "! (Max is " + 31 + ")");
      ...
      DataWatcher.WatchableObject datawatcher$watchableobject =
          new DataWatcher.WatchableObject(integer.intValue(), id, object);
  ...
  public void addObjectByDataType(int id, int type) {
    DataWatcher.WatchableObject datawatcher$watchableobject =
        new DataWatcher.WatchableObject(type, id, (Object) null);
    this.lock.writeLock().lock();
    this.watchedObjects.put(Integer.valueOf(id), datawatcher$watchableobject);
    this.lock.writeLock().unlock();
    this.isBlank = false;
  ...
  /**
   * Writes the list of watched objects (entity attribute of type {byte, short, int, float, string,
   * ItemStack, ChunkCoordinates}) to the specified PacketBuffer
   */
  public static void writeWatchedListToPacketBuffer(
      List<DataWatcher.WatchableObject> objectsList, PacketBuffer buffer) throws IOException {
    if (objectsList != null) {
  ...
  /**
   * Writes a watchable object (entity attribute of type {byte, short, int, float, string,
   * ItemStack, ChunkCoordinates}) to the specified PacketBuffer
   */
  private static void writeWatchableObjectToPacketBuffer(
      PacketBuffer buffer, DataWatcher.WatchableObject object) throws IOException {
  ...
  public static List<DataWatcher.WatchableObject> readWatchedListFromPacketBuffer(
    ...
    for (int i = buffer.readByte(); i != 127; i = buffer.readByte()) {
      ...
      switch (j) {
        case 0:
          datawatcher$watchableobject =
              new DataWatcher.WatchableObject(j, k, Byte.valueOf(buffer.readByte()));
          break;

        case 1:
          datawatcher$watchableobject =
              new DataWatcher.WatchableObject(j, k, Short.valueOf(buffer.readShort()));
          break;

        case 2:
          datawatcher$watchableobject =
              new DataWatcher.WatchableObject(j, k, Integer.valueOf(buffer.readInt()));
          break;

        case 3:
          datawatcher$watchableobject =
              new DataWatcher.WatchableObject(j, k, Float.valueOf(buffer.readFloat()));
          break;

        case 4:
          datawatcher$watchableobject =
              new DataWatcher.WatchableObject(j, k, buffer.readStringFromBuffer(32767));
          break;

        case 5:
          datawatcher$watchableobject =
              new DataWatcher.WatchableObject(j, k, buffer.readItemStackFromBuffer());
          break;
        ...
        case 6:
          ...
          datawatcher$watchableobject =
              new DataWatcher.WatchableObject(j, k, new BlockPos(l, i1, j1));
          break;
        ...
        case 7:
          ...
          datawatcher$watchableobject =
              new DataWatcher.WatchableObject(j, k, new Rotations(f, f1, f2));
      }
  ...
  public void updateWatchedObjectsFromList(List<DataWatcher.WatchableObject> p_75687_1_) {
    ...
    for (DataWatcher.WatchableObject datawatcher$watchableobject : p_75687_1_) {
      DataWatcher.WatchableObject datawatcher$watchableobject1 =
          (DataWatcher.WatchableObject)
              this.watchedObjects.get(
                  Integer.valueOf(datawatcher$watchableobject.getDataValueId()));
