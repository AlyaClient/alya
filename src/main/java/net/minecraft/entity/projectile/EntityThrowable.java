...
public abstract class EntityThrowable extends Entity implements IProjectile {
  ...
  public EntityThrowable(World worldIn, EntityLivingBase throwerIn) {
    ...
    this.motionY =
        (double)
            (-MathHelper.sin((this.rotationPitch + this.getInaccuracy()) / 180.0F * (float) Math.PI)
                * f);
  ...
  public void onUpdate() {
      ...
      for (int j = 0; j < list.size(); ++j) {
        ...
        if (entity1.canBeCollidedWith() && (entity1 != entitylivingbase || this.ticksInAir >= 5)) {
          ...
          MovingObjectPosition movingobjectposition1 =
              axisalignedbb.calculateIntercept(vec3, vec31);
    ...
    if (movingobjectposition != null) {
      if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK
          && this.worldObj.getBlockState(movingobjectposition.getBlockPos()).getBlock()
              == Blocks.portal) {
    ...
    for (this.rotationPitch =
            (float) (MathHelper.func_181159_b(this.motionY, (double) f1) * 180.0D / Math.PI);
  ...
  public EntityLivingBase getThrower() {
    if (this.thrower == null && this.throwerName != null && this.throwerName.length() > 0) {
      ...
      if (this.thrower == null && this.worldObj instanceof WorldServer) {
        try {
          Entity entity =
              ((WorldServer) this.worldObj).getEntityFromUuid(UUID.fromString(this.throwerName));
