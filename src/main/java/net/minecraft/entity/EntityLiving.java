...
public abstract class EntityLiving extends EntityLivingBase {
  ...
  public void setAttackTarget(EntityLivingBase entitylivingbaseIn) {
    ...
    Reflector.callVoid(
        Reflector.ForgeHooks_onLivingSetAttackTarget, new Object[] {this, entitylivingbaseIn});
  }
    ...
    if (this.leashedToEntity != null) {
      ...
      if (this.leashedToEntity instanceof EntityLivingBase) {
        nbttagcompound1.setLong(
            "UUIDMost", this.leashedToEntity.getUniqueID().getMostSignificantBits());
        nbttagcompound1.setLong(
            "UUIDLeast", this.leashedToEntity.getUniqueID().getLeastSignificantBits());
        ...
        if (i == 0) {
          if (itemstack.getItem() instanceof ItemSword
            ...
            if (itemsword.getDamageVsEntity() != itemsword1.getDamageVsEntity()) {
              ...
              && itemstack1.getItem() instanceof ItemBow) {
  ...
  protected void despawnEntity() {
    ...
    if (this.persistenceRequired) {
      this.entityAge = 0;
        ...
        && (object =
                Reflector.call(Reflector.ForgeEventFactory_canEntityDespawn, new Object[] {this}))
            != object1) {
      if (object == object2) {
        this.entityAge = 0;
      } else {
      ...
      if (entity != null) {
        ...
        if (this.entityAge > 600
            && this.rand.nextInt(800) == 0
            && d3 > 1024.0D
            && this.canDespawn()) {
          this.setDead();
        } else if (d3 < 1024.0D) {
  ...
  /**
   * The speed it takes to move the entityliving's rotationPitch through the faceEntity method. This
   * is only currently use in wolves.
   */
  ...
  /**
   * Changes pitch and yaw so that the entity calling the function is facing the entity provided as
   * an argument.
   */
  public void faceEntity(Entity entityIn, float p_70625_2_, float p_70625_3_) {
    ...
    if (entityIn instanceof EntityLivingBase) {
      ...
      d2 =
          (entityIn.getEntityBoundingBox().minY + entityIn.getEntityBoundingBox().maxY) / 2.0D
              - (this.posY + (double) this.getEyeHeight());
    }
  ...
  protected void setEnchantmentBasedOnDifficulty(DifficultyInstance difficulty) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (itemstack != null && this.rand.nextFloat() < 0.5F * f) {
        EnchantmentHelper.addRandomEnchantment(
            this.rand, itemstack, (int) (5.0F + f * (float) this.rand.nextInt(18)));
      }
    }
  }
  ...
  public IEntityLivingData onInitialSpawn(
    ...
    this.getEntityAttribute(SharedMonsterAttributes.followRange)
        .applyModifier(
            new AttributeModifier("Random spawn bonus", this.rand.nextGaussian() * 0.05D, 1));
    return livingdata;
  }

  /**
   * returns true if all the conditions for steering the entity are met. For pigs, this is true if
   * it is being ridden by a player and the player is holding a carrot-on-a-stick
   */
    ...
    if (this.isLeashed) {
      ...
      if (!this.worldObj.isRemote && sendPacket && this.worldObj instanceof WorldServer) {
        ((WorldServer) this.worldObj)
            .getEntityTracker()
            .sendToAllTrackingEntity(this, new S1BPacketEntityAttach(1, this, (Entity) null));
      }
    }
  }
  ...
  public void setLeashedToEntity(Entity entityIn, boolean sendAttachNotification) {
    ...
    if (!this.worldObj.isRemote && sendAttachNotification && this.worldObj instanceof WorldServer) {
      ((WorldServer) this.worldObj)
          .getEntityTracker()
          .sendToAllTrackingEntity(this, new S1BPacketEntityAttach(1, this, this.leashedToEntity));
    }
  }
  ...
  private void recreateLeash() {
    if (this.isLeashed && this.leashNBTTag != null) {
      if (this.leashNBTTag.hasKey("UUIDMost", 4) && this.leashNBTTag.hasKey("UUIDLeast", 4)) {
        UUID uuid =
            new UUID(this.leashNBTTag.getLong("UUIDMost"), this.leashNBTTag.getLong("UUIDLeast"));

        for (EntityLivingBase entitylivingbase :
            this.worldObj.getEntitiesWithinAABB(
                EntityLivingBase.class, this.getEntityBoundingBox().expand(10.0D, 10.0D, 10.0D))) {
          if (entitylivingbase.getUniqueID().equals(uuid)) {
            this.leashedToEntity = entitylivingbase;
            break;
          }
        }
      } else if (this.leashNBTTag.hasKey("X", 99)
          && this.leashNBTTag.hasKey("Y", 99)
        ...
        EntityLeashKnot entityleashknot =
            EntityLeashKnot.getKnotForPosition(this.worldObj, blockpos);
  ...
  public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn) {
    ...
    if (itemStackIn == null
        || getArmorPosition(itemStackIn) == i
