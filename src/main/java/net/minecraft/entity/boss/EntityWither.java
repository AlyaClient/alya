...
public class EntityWither extends EntityMob implements IBossDisplayData, IRangedAttackMob {
      ...
      new Predicate<Entity>() {
        public boolean apply(Entity p_apply_1_) {
          return p_apply_1_ instanceof EntityLivingBase
              && ((EntityLivingBase) p_apply_1_).getCreatureAttribute()
  ...
  public EntityWither(World worldIn) {
    ...
    this.targetTasks.addTask(
        ...
        new EntityAINearestAttackableTarget(
            this, EntityLiving.class, 0, false, false, attackEntitySelector));
  ...
  public void onLivingUpdate() {
    ...
    if (this.motionX * this.motionX + this.motionZ * this.motionZ > 0.05000000074505806D) {
      this.rotationYaw =
          (float) MathHelper.func_181159_b(this.motionZ, this.motionX) * (180F / (float) Math.PI)
              - 90.0F;
    }
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      if (entity1 != null) {
        ...
        this.field_82221_e[j] =
            this.func_82204_b(this.field_82221_e[j], this.renderYawOffset, 10.0F);
      }
    }
        ...
        if (this.ticksExisted >= this.field_82223_h[i - 1]) {
          ...
          if (k1 > 0) {
            ...
            List<EntityLivingBase> list =
                this.worldObj.<EntityLivingBase>getEntitiesWithinAABB(
                    EntityLivingBase.class,
                    this.getEntityBoundingBox().expand(20.0D, 8.0D, 20.0D),
                    Predicates.<EntityLivingBase>and(
                        attackEntitySelector, EntitySelectors.NOT_SPECTATING));
  ...
  private double func_82214_u(int p_82214_1_) {
    if (p_82214_1_ <= 0) {
      ...
      float f =
          (this.renderYawOffset + (float) (180 * (p_82214_1_ - 1))) / 180.0F * (float) Math.PI;
  ...
  private double func_82213_w(int p_82213_1_) {
    if (p_82213_1_ <= 0) {
      ...
      float f =
          (this.renderYawOffset + (float) (180 * (p_82213_1_ - 1))) / 180.0F * (float) Math.PI;
  ...
  private void launchWitherSkullToCoords(
      int p_82209_1_, double x, double y, double z, boolean invulnerable) {
    ...
    if (!this.worldObj.isRemote) {
      for (EntityPlayer entityplayer :
          this.worldObj.getEntitiesWithinAABB(
              EntityPlayer.class, this.getEntityBoundingBox().expand(50.0D, 100.0D, 50.0D))) {
  ...
  /**
   * Returns the target entity ID if present, or -1 if not @param par1 The target offset, should be
   * from 0-2
   */
  ...
  /**
   * Returns whether the wither is armored with its boss armor or not by checking whether its health
   * is below half of its maximum.
   */
