...
public class EntityDragon extends EntityLiving implements IBossDisplayData, IEntityMultiPart, IMob {
  ...
  /**
   * Ring buffer array for the last 64 Y-positions and yaw rotations. Used to calculate offsets for
   * the animations.
   */
  public double[][] ringBuffer = new double[64][3];

  /**
   * Index into the ring buffer. Incremented once per tick and restarts at 0 once it reaches the end
   * of the buffer.
   */
  ...
  /**
   * Animation time, used to control the speed of the animation cycles (wings flapping, jaw opening,
   * etc.)
   */
  ...
  /**
   * Activated if the dragon is flying though obsidian, white stone or bedrock. Slows movement and
   * animation speed.
   */
  ...
  /**
   * Returns a double[3] array with movement offsets, used to calculate trailing tail/neck
   * positions. [0] = yaw offset, [1] = y offset, [2] = unused, always 0. Parameters: buffer index
  ...
  public double[] getMovementOffsets(int p_70974_1_, float p_70974_2_) {
    ...
    adouble[2] =
        this.ringBuffer[i][2]
            + (this.ringBuffer[j][2] - this.ringBuffer[i][2]) * (double) p_70974_2_;
    return adouble;
  }
  ...
  public void onLivingUpdate() {
    ...
    if (this.getHealth() <= 0.0F) {
      ...
      this.worldObj.spawnParticle(
          ...
          this.posX + (double) f11,
          this.posY + 2.0D + (double) f13,
          this.posZ + (double) f14,
          0.0D,
          0.0D,
          0.0D,
          new int[0]);
      ...
      if (this.isAIDisabled()) {
        ...
        if (this.worldObj.isRemote) {
          if (this.newPosRotationIncrements > 0) {
            ...
            double d0 =
                this.posY + (this.newPosY - this.posY) / (double) this.newPosRotationIncrements;
            double d1 =
                this.posZ + (this.newPosZ - this.posZ) / (double) this.newPosRotationIncrements;
            double d2 =
                MathHelper.wrapAngleTo180_double(this.newRotationYaw - (double) this.rotationYaw);
            this.rotationYaw =
          ...
          this.randomYawVelocity =
              (float)
                  ((double) this.randomYawVelocity + d6 * (0.699999988079071D / d9 / (double) f6));
        ...
        if (!this.worldObj.isRemote && this.hurtTime == 0) {
          ...
          this.attackEntitiesInList(
              this.worldObj.getEntitiesWithinAABBExcludingEntity(
                  this, this.dragonPartHead.getEntityBoundingBox().expand(1.0D, 1.0D, 1.0D)));
        }
        ...
        if (!this.worldObj.isRemote) {
          this.slowed =
              this.destroyBlocksInAABB(this.dragonPartHead.getEntityBoundingBox())
  ...
  private void updateDragonEnderCrystal() {
    ...
    if (this.rand.nextInt(10) == 0) {
      float f = 32.0F;
      List<EntityEnderCrystal> list =
          this.worldObj.<EntityEnderCrystal>getEntitiesWithinAABB(
  ...
  private void collideWithEntities(List<Entity> p_70970_1_) {
    double d0 =
        (this.dragonPartBody.getEntityBoundingBox().minX
                + this.dragonPartBody.getEntityBoundingBox().maxX)
            / 2.0D;
    double d1 =
        (this.dragonPartBody.getEntityBoundingBox().minZ
                + this.dragonPartBody.getEntityBoundingBox().maxZ)
            / 2.0D;
  ...
  /**
   * Simplifies the value of a number by adding/subtracting 180 to the point that the number is
   * between -180 and 180.
   */
  ...
  private boolean destroyBlocksInAABB(AxisAlignedBB p_70972_1_) {
    ...
    if (flag1) {
      double d0 =
          p_70972_1_.minX + (p_70972_1_.maxX - p_70972_1_.minX) * (double) this.rand.nextFloat();
      double d1 =
          p_70972_1_.minY + (p_70972_1_.maxY - p_70972_1_.minY) * (double) this.rand.nextFloat();
      double d2 =
          p_70972_1_.minZ + (p_70972_1_.maxZ - p_70972_1_.minZ) * (double) this.rand.nextFloat();
      this.worldObj.spawnParticle(
          EnumParticleTypes.EXPLOSION_LARGE, d0, d1, d2, 0.0D, 0.0D, 0.0D, new int[0]);
    }
  ...
  public boolean attackEntityFromPart(
      EntityDragonPart dragonPart, DamageSource source, float p_70965_3_) {
    ...
    this.targetX =
        this.posX + (double) (f1 * 5.0F) + (double) ((this.rand.nextFloat() - 0.5F) * 2.0F);
    this.targetY = this.posY + (double) (this.rand.nextFloat() * 3.0F) + 1.0D;
    this.targetZ =
        this.posZ - (double) (f2 * 5.0F) + (double) ((this.rand.nextFloat() - 0.5F) * 2.0F);
    this.target = null;
      ...
      if (this.deathTicks > 150 && this.deathTicks % 5 == 0 && flag) {
        ...
        while (i > 0) {
          ...
          this.worldObj.spawnEntityInWorld(
              new EntityXPOrb(this.worldObj, this.posX, this.posY, this.posZ, k));
        }
      }
    ...
    if (this.deathTicks == 200 && !this.worldObj.isRemote) {
      if (flag) {
        ...
        while (j > 0) {
          ...
          j -= l;
          this.worldObj.spawnEntityInWorld(
              new EntityXPOrb(this.worldObj, this.posX, this.posY, this.posZ, l));
        }
      }
