...
public abstract class Entity implements ICommandSender {
  ...
  /**
   * Adds 15% to the entity's yaw and subtracts 15% from the pitch. Clamps pitch from -90 to 90.
  ...
  private boolean isLiquidPresentInAABB(AxisAlignedBB bb) {
    return this.worldObj.getCollidingBoundingBoxes(this, bb).isEmpty()
    ...
    if (this.noClip) {
      ...
      if (flag) {
            ...
            x != 0.0D
                && this.worldObj
                    .getCollidingBoundingBoxes(
                        this, this.getEntityBoundingBox().offset(x, -1.0D, 0.0D))
                    .isEmpty();
            d3 = x) {
          if (x < d6 && x >= -d6) {
            x = 0.0D;
        ...
        for (;
            z != 0.0D
                && this.worldObj
                    .getCollidingBoundingBoxes(
                        this, this.getEntityBoundingBox().offset(0.0D, -1.0D, z))
                    .isEmpty();
            d5 = z) {
          if (z < d6 && z >= -d6) {
            z = 0.0D;
        ...
        for (;
            x != 0.0D
                ...
                && this.worldObj
                    .getCollidingBoundingBoxes(
                        this, this.getEntityBoundingBox().offset(x, -1.0D, z))
                    .isEmpty();
            d5 = z) {
          if (x < d6 && x >= -d6) {
            x = 0.0D;
      ...
      List<AxisAlignedBB> list1 =
          this.worldObj.getCollidingBoundingBoxes(
              this, this.getEntityBoundingBox().addCoord(x, y, z));
      ...
      if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
        ...
        List<AxisAlignedBB> list =
            this.worldObj.getCollidingBoundingBoxes(
                this, this.getEntityBoundingBox().addCoord(d3, y, d5));
      ...
      if (this.canTriggerWalking() && !flag && this.ridingEntity == null) {
        ...
        this.distanceWalkedOnStepModified =
            (float)
                ((double) this.distanceWalkedOnStepModified
          ...
          if (this.isInWater()) {
            float f =
                MathHelper.sqrt_double(
                        this.motionX * this.motionX * 0.20000000298023224D
      ...
      try {
        ...
        CrashReport crashreport =
            CrashReport.makeCrashReport(throwable, "Checking entity block collision");
      ...
      if (this.worldObj.isFlammableWithin(
          this.getEntityBoundingBox().contract(0.001D, 0.001D, 0.001D))) {
      ...
      if (flag2 && this.fire > 0) {
        this.playSound(
            "random.fizz", 0.7F, 1.6F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4F);
        this.fire = -this.fireResistance;
      }
  ...
  /**
   * Resets the entity's position to the center (planar) and bottom (vertical) points of its
   * bounding box.
   */
        ...
        for (int j = blockpos.getY(); j <= blockpos1.getY(); ++j) {
          for (int k = blockpos.getZ(); k <= blockpos1.getZ(); ++k) {
            ...
            try {
              iblockstate
                  .getBlock()
                  .onEntityCollidedWithBlock(this.worldObj, blockpos2, iblockstate, this);
            } catch (Throwable throwable) {
              CrashReport crashreport =
  ...
  public boolean isWet() {
    return this.inWater
        ...
        || this.worldObj.canLightningStrike(
  ...
  protected void resetHeight() {
    ...
    this.playSound(
        this.getSplashSound(), f, 1.0F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4F);
  ...
  public boolean isInsideOfMaterial(Material materialIn) {
    ...
    if (block.getMaterial() == materialIn) {
      float f =
          BlockLiquid.getLiquidHeightPercent(iblockstate.getBlock().getMetaFromState(iblockstate))
              - 0.11111111F;
  ...
  public float getBrightness(float partialTicks) {
    ...
    return this.worldObj.isBlockLoaded(blockpos)
        ? this.worldObj.getLightBrightness(blockpos)
        : 0.0F;
  }
  ...
  public MovingObjectPosition rayTrace(
      double blockReachDistance, final float yaw, final float pitch) {
  ...
  /**
   * Like writeToNBTOptional but does not check if the entity is ridden. Used for saving ridden
   * entities with their riders.
   */
  ...
  /**
   * Either write this entity to the NBT tag given and return true, or return false without doing
   * anything. If this returns false the entity is not saved on disk. Ridden entities return false
   * here as they are saved with their rider.
   */
  ...
  public void writeToNBT(NBTTagCompound tagCompund) {
    try {
      tagCompund.setTag(
          "Pos", this.newDoubleNBTList(new double[] {this.posX, this.posY, this.posZ}));
      tagCompund.setTag(
          "Motion", this.newDoubleNBTList(new double[] {this.motionX, this.motionY, this.motionZ}));
      tagCompund.setTag(
          "Rotation", this.newFloatNBTList(new float[] {this.rotationYaw, this.rotationPitch}));
  ...
  public void readFromNBT(NBTTagCompound tagCompund) {
    try {
      ...
      if (tagCompund.hasKey("UUIDMost", 4) && tagCompund.hasKey("UUIDLeast", 4)) {
        this.entityUniqueID =
            new UUID(tagCompund.getLong("UUIDMost"), tagCompund.getLong("UUIDLeast"));
  ...
  public EntityItem entityDropItem(ItemStack itemStackIn, float offsetY) {
    if (itemStackIn.stackSize != 0 && itemStackIn.getItem() != null) {
      EntityItem entityitem =
          new EntityItem(
              this.worldObj, this.posX, this.posY + (double) offsetY, this.posZ, itemStackIn);
  ...
  public boolean isEntityInsideOpaqueBlock() {
    if (this.noClip) {
      ...
      BlockPos.MutableBlockPos blockpos$mutableblockpos =
          new BlockPos.MutableBlockPos(Integer.MIN_VALUE, Integer.MIN_VALUE, Integer.MIN_VALUE);
      ...
      for (int i = 0; i < 8; ++i) {
        ...
        if (blockpos$mutableblockpos.getX() != k
            || blockpos$mutableblockpos.getY() != j
  ...
  public void updateRidden() {
    if (this.ridingEntity.isDead) {
      ...
      if (this.ridingEntity != null) {
        ...
        this.entityRiderYawDelta +=
            (double) (this.ridingEntity.rotationYaw - this.ridingEntity.prevRotationYaw);

        for (this.entityRiderPitchDelta +=
                (double) (this.ridingEntity.rotationPitch - this.ridingEntity.prevRotationPitch);
  ...
  public void setPositionAndRotation2(
    ...
    List<AxisAlignedBB> list =
        this.worldObj.getCollidingBoundingBoxes(
            this, this.getEntityBoundingBox().contract(0.03125D, 0.0D, 0.03125D));
    ...
    if (this.timeUntilPortal > 0) {
      ...
      if (!this.worldObj.isRemote && !p_181015_1_.equals(this.field_181016_an)) {
        ...
        BlockPattern.PatternHelper blockpattern$patternhelper =
            Blocks.portal.func_181089_f(this.worldObj, p_181015_1_);
        double d0 =
            blockpattern$patternhelper.getFinger().getAxis() == EnumFacing.Axis.X
        ...
        double d2 =
            MathHelper.func_181160_c(
                ...
                (double)
                    (blockpattern$patternhelper.func_181117_a().getY()
  ...
  /**
   * Returns true if the flag is active for the entity. Known flags: 0) is burning; 1) is sneaking;
   * 2) is riding something; 3) is sprinting; 4) is eating
   */
  ...
  public void copyLocationAndAnglesFrom(Entity entityIn) {
    this.setLocationAndAngles(
        entityIn.posX, entityIn.posY, entityIn.posZ, entityIn.rotationYaw, entityIn.rotationPitch);
  }
  ...
  public void travelToDimension(int dimensionId) {
    if (!this.worldObj.isRemote && !this.isDead) {
      ...
      minecraftserver
          ...
          .transferEntityToWorld(this, i, worldserver, worldserver1);
  ...
  public float getExplosionResistance(
      Explosion explosionIn, World worldIn, BlockPos pos, IBlockState blockStateIn) {
  ...
  public void addEntityCrashInfo(CrashReportCategory category) {
    ...
    category.addCrashSection(
        ...
        CrashReportCategory.getCoordinateInfo(
            (double) MathHelper.floor_double(this.posX),
            (double) MathHelper.floor_double(this.posY),
            (double) MathHelper.floor_double(this.posZ)));
    category.addCrashSection(
        "Entity\'s Momentum",
        String.format(
            "%.2f, %.2f, %.2f",
  ...
  protected HoverEvent getHoverEvent() {
    ...
    return new HoverEvent(
        HoverEvent.Action.SHOW_ENTITY, new ChatComponentText(nbttagcompound.toString()));
  }
  ...
  /**
   * Get the world, if available. <b>{@code null} is not allowed!</b> If you are not an entity in
   * the world, return the overworld
   */
  ...
  /**
   * New version of interactWith that includes vector information on where precisely the player
   * targeted.
   */
