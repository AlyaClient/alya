...
public class EntityArmorStand extends EntityLivingBase {
  ...
  public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn) {
    ...
    if (itemStackIn != null
        && EntityLiving.getArmorPosition(itemStackIn) != i
    ...
    if (list != null && !list.isEmpty()) {
      for (int i = 0; i < list.size(); ++i) {
        ...
        if (entity instanceof EntityMinecart
            && ((EntityMinecart) entity).getMinecartType()
                == EntityMinecart.EnumMinecartType.RIDEABLE
  ...
  public boolean interactAt(EntityPlayer player, Vec3 targetVec3) {
    if (this.func_181026_s()) {
      ...
      if (d3 >= 0.1D && d3 < 0.1D + (flag1 ? 0.8D : 0.45D) && this.contents[1] != null) {
        j = 1;
      } else if (d3 >= 0.9D + (flag1 ? 0.3D : 0.0D)
          && d3 < 0.9D + (flag1 ? 1.0D : 0.7D)
