...
public class ScreenShotHelper {
  ...
  public static IChatComponent saveScreenshot(
      File gameDirectory, int width, int height, Framebuffer buffer) {
   ...
   */
  public static IChatComponent saveScreenshot(
      File gameDirectory, String screenshotName, int width, int height, Framebuffer buffer) {
    try {
        ...
        for (int j1 = i1; j1 < buffer.framebufferTextureHeight; ++j1) {
          for (int k1 = 0; k1 < buffer.framebufferWidth; ++k1) {
            bufferedimage.setRGB(
                k1, j1 - i1, pixelValues[j1 * buffer.framebufferTextureWidth + k1]);
          }
        }
      } else {
      ...
      return new ChatComponentTranslation(
          "screenshot.failure", new Object[] {exception.getMessage()});
    }
  }

  /**
   * Creates a unique PNG file in the given directory named by a timestamp. Handles cases where the
   * timestamp alone is not enough to create a uniquely named file, though it still might suffer
   * from an unlikely race condition where the filename was unique when this method was called, but
   * another process or thread created a file at the same path immediately after this method
   * returned.
   */
  ...
  private static void resize(int p_resize_0_, int p_resize_1_) {
    ...
    if (minecraft.currentScreen != null) {
      ScaledResolution scaledresolution = new ScaledResolution(minecraft);
      minecraft.currentScreen.onResize(
          minecraft, scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight());
    }
