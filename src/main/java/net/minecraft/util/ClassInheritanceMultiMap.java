...
public class ClassInheritanceMultiMap<T> extends AbstractSet<T> {
  private static final Set<Class<?>> field_181158_a =
      Collections.<Class<?>>newSetFromMap(new ConcurrentHashMap());
  ...
  public <S> Iterable<S> getByClass(final Class<S> clazz) {
    return new Iterable<S>() {
      public Iterator<S> iterator() {
        List<T> list =
            (List)
                ClassInheritanceMultiMap.this.map.get(
                    ClassInheritanceMultiMap.this.func_181157_b(clazz));
  ...
  public Iterator<T> iterator() {
    return (Iterator<T>)
        (this.field_181745_e.isEmpty()
            ? Collections.emptyIterator()
            : IteratorCache.getReadOnly(this.field_181745_e));
  }
