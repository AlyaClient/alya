...
public class ChatComponentTranslation extends ChatComponentStyle {
  ...
  /**
   * initializes our children from a format string, using the format args to fill in the placeholder
   * variables.
   */
    ...
    try {
      ...
      for (; matcher.find(j); j = l) {
        ...
        if (k > j) {
          ChatComponentText chatcomponenttext =
              new ChatComponentText(String.format(format.substring(j, k), new Object[0]));
        ...
        if ("%".equals(s2) && "%%".equals(s)) {
          ...
          if (!"s".equals(s2)) {
            throw new ChatComponentTranslationFormatException(
                this, "Unsupported format: \'" + s + "\'");
          }
      ...
      if (j < format.length()) {
        ChatComponentText chatcomponenttext1 =
            new ChatComponentText(String.format(format.substring(j), new Object[0]));
  ...
  public Iterator<IChatComponent> iterator() {
    ...
    return Iterators.<IChatComponent>concat(
        createDeepCopyIterator(this.children), createDeepCopyIterator(this.siblings));
  }

  /**
   * Gets the text of this component, without any special formatting codes added, for chat. TODO:
   * why is this two different methods?
   */
  ...
  public ChatComponentTranslation createCopy() {
    ...
    ChatComponentTranslation chatcomponenttranslation =
        new ChatComponentTranslation(this.key, aobject);
