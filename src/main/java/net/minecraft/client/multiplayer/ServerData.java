...
public class ServerData {
  ...
  /**
   * the string indicating number of players on and capacity of the server that is shown on the
   * server browser (i.e. "5/20" meaning 5 slots used out of 20 slots total)
   */
  public String populationInfo;

  /**
   * (better variable name would be 'hostname') server name as displayed in the server browser's
   * second line (grey text)
   */
  ...
  public static ServerData getServerDataFromNBTCompound(NBTTagCompound nbtCompound) {
    ServerData serverdata =
        new ServerData(nbtCompound.getString("name"), nbtCompound.getString("ip"), false);
  ...
  public static enum ServerResourceMode {
    ...
    private ServerResourceMode(String p_i1053_3_) {
      this.motd =
          new ChatComponentTranslation("addServer.resourcePack." + p_i1053_3_, new Object[0]);
    }
