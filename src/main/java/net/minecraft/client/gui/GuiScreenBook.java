...
public class GuiScreenBook extends GuiScreen {
  ...
  public void initGui() {
    ...
    int j = 2;
    this.buttonList.add(
        this.buttonNextPage = new GuiScreenBook.NextPageButton(1, i + 120, j + 154, true));
    this.buttonList.add(
        this.buttonPreviousPage = new GuiScreenBook.NextPageButton(2, i + 38, j + 154, false));
    this.updateButtons();
  }
  ...
  private void updateButtons() {
    this.buttonNextPage.visible =
        !this.bookGettingSigned && (this.currPage < this.bookTotalPages - 1 || this.bookIsUnsigned);
  ...
  private void sendBookToServer(boolean publish) throws IOException {
    if (this.bookIsUnsigned && this.bookIsModified) {
      if (this.bookPages != null) {
        ...
        if (publish) {
          s2 = "MC|BSign";
          this.bookObj.setTagInfo(
              "author", new NBTTagString(this.editingPlayer.getCommandSenderName()));
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    if (this.bookGettingSigned) {
      ...
      font.drawString(s1, i + 36 + (116 - k) / 2, j + 16 + 16, 0);
      int l = (int) font.getStringWidth(s);
      font.drawString(s, i + 36 + (116 - l) / 2, j + 48, 0);
      String s2 =
          I18n.format("book.byAuthor", new Object[] {this.editingPlayer.getCommandSenderName()});
      int i1 = (int) font.getStringWidth(s2);
      font.drawString(EnumChatFormatting.DARK_GRAY + s2, i + 36 + (116 - i1) / 2, j + 48 + 10, 0);
      ...
      if (this.bookIsUnsigned) {
        ...
        if (ItemEditableBook.validBookTagContents(this.bookObj.getTagCompound())) {
          try {
            ...
            this.field_175386_A =
                ichatcomponent != null
                    ? GuiUtilRenderComponents.func_178908_a(
                        ichatcomponent, 116, this.fontRendererObj, true, true)
                    : null;
      ...
      int j1 = (int) font.getStringWidth(s4);
      font.drawString(s4, i - j1 + this.bookImageWidth - 44, j + 16, 0);

      if (this.field_175386_A == null) {
        this.fontRendererObj.drawSplitString(s5, i + 36, j + 16 + 16, 116, 0);
      } else {
        int k1 = Math.min(128 / (int) font.getFontHeight(), this.field_175386_A.size());

        for (int l1 = 0; l1 < k1; ++l1) {
          IChatComponent ichatcomponent2 = (IChatComponent) this.field_175386_A.get(l1);
          font.drawString(
              ichatcomponent2.getUnformattedText(),
              i + 36,
              j + 16 + 16 + l1 * (int) font.getFontHeight(),
              0);
        }
  ...
  protected boolean handleComponentClick(IChatComponent p_175276_1_) {
    ClickEvent clickevent =
        p_175276_1_ == null ? null : p_175276_1_.getChatStyle().getChatClickEvent();
  ...
  public IChatComponent func_175385_b(int p_175385_1_, int p_175385_2_) {
    if (this.field_175386_A == null) {
      ...
      if (i >= 0 && j >= 0) {
        int k = Math.min(128 / (int) font.getFontHeight(), this.field_175386_A.size());

        if (i <= 116 && j < (int) font.getFontHeight() * k + k) {
          int l = j / (int) font.getFontHeight();
