...
public class GuiDisconnected extends GuiScreen {
  ...
  public void initGui() {
    ...
    this.multilineMessage =
        this.fontRendererObj.listFormattedStringToWidth(
            this.message.getFormattedText(), this.width - 50);
    this.field_175353_i = this.multilineMessage.size() * (int) font.getFontHeight();
    this.buttonList.add(
        new GuiButton(
            0,
            this.width / 2 - 100,
            this.height / 2 + this.field_175353_i / 2 + (int) font.getFontHeight(),
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    this.drawCenteredString(
        this.reason,
        this.width / 2,
        this.height / 2 - this.field_175353_i / 2 - (int) font.getFontHeight() * 2,
        11184810);
