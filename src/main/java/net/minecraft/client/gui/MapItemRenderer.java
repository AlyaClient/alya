...
public class MapItemRenderer {
  ...
  class Instance {
    ...
    private Instance(MapData mapdataIn) {
      ...
      this.location =
          MapItemRenderer.this.textureManager.getDynamicTextureLocation(
              "map/" + mapdataIn.mapName, this.mapTexture);
    ...
    private void render(boolean noOverlayRendering) {
      ...
      worldrenderer
          .pos(
              (double) ((float) (i + 0) + f),
              (double) ((float) (j + 128) - f),
              -0.009999999776482582D)
          .tex(0.0D, 1.0D)
          .endVertex();
      worldrenderer
          .pos(
              (double) ((float) (i + 128) - f),
              (double) ((float) (j + 128) - f),
              -0.009999999776482582D)
          .tex(1.0D, 1.0D)
          .endVertex();
      ...
      for (Vec4b vec4b : this.mapData.mapDecorations.values()) {
        if (!noOverlayRendering || vec4b.func_176110_a() == 1) {
          ...
          GlStateManager.translate(
              (float) i + (float) vec4b.func_176112_b() / 2.0F + 64.0F,
              (float) j + (float) vec4b.func_176113_c() / 2.0F + 64.0F,
              -0.02F);
