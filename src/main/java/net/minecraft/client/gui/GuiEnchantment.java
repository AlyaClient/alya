...
public class GuiEnchantment extends GuiContainer {
  ...
  protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
    font.drawString(
        this.field_175380_I.getDisplayName().getUnformattedText(),
        (float) (12),
        (float) (5),
        4210752);
    font.drawString(
        this.playerInventory.getDisplayName().getUnformattedText(),
        (float) (8),
        (float) (this.ySize - 96 + 2),
        4210752);
  }
  ...
  protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      if (l >= 0
          ...
          && l < 108
          && i1 < 19
          && this.container.enchantItem(this.mc.thePlayer, k)) {
  ...
  protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
    ...
    float f3 =
        this.field_147069_w + (this.field_147071_v - this.field_147069_w) * partialTicks + 0.25F;
    float f4 =
        this.field_147069_w + (this.field_147071_v - this.field_147069_w) * partialTicks + 0.75F;
    ...
    for (int l = 0; l < 3; ++l) {
      ...
      if (l1 == 0) {
        ...
        fontrenderer.drawStringWithShadow(
            s1,
            (float) (j1 + 86 - fontrenderer.getStringWidth(s1)),
            (float) (j + 16 + 19 * l + 7),
            i2);
      }
    }
