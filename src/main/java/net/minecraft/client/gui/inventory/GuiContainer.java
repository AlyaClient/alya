...
public abstract class GuiContainer extends GuiScreen {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    ItemStack itemstack =
        this.draggedStack == null ? inventoryplayer.getItemStack() : this.draggedStack;
  ...
  private void drawItemStack(ItemStack stack, int x, int y, String altText) {
    ...
    this.itemRender.renderItemOverlayIntoGUI(
        this.fontRendererObj, stack, x, y - (this.draggedStack == null ? 0 : 8), altText);
    this.zLevel = 0.0F;
  ...
  private void drawSlot(Slot slotIn) {
    ...
    boolean flag = false;
    boolean flag1 =
        slotIn == this.clickedSlot && this.draggedStack != null && !this.isRightMouseClick;
    ItemStack itemstack1 = this.mc.thePlayer.inventory.getItemStack();
    String s = null;

    if (slotIn == this.clickedSlot
      ...
      itemstack.stackSize /= 2;
        ...
        && this.dragSplittingSlots.contains(slotIn)
        && itemstack1 != null) {
  ...
  protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    ...
    if (mouseButton == 0 || mouseButton == 1 || flag) {
      ...
      int k = this.guiTop;
      boolean flag1 =
          mouseX < j || mouseY < k || mouseX >= j + this.xSize || mouseY >= k + this.ySize;
      int l = -1;
  ...
  protected void mouseClickMove(
    ...
    if (this.clickedSlot != null && this.mc.gameSettings.touchscreen) {
      if (clickedMouseButton == 0 || clickedMouseButton == 1) {
        ...
        } else if (this.draggedStack.stackSize > 1
            && slot != null
    ...
    if (this.doubleClick
      ...
      if (this.clickedSlot != null && this.mc.gameSettings.touchscreen) {
        ...
        this.handleMouseClick(
            (Slot) null, -999, Container.func_94534_d(0, this.dragSplittingLimit), 5);

        for (Slot slot1 : this.dragSplittingSlots) {
          this.handleMouseClick(
              slot1, slot1.slotNumber, Container.func_94534_d(1, this.dragSplittingLimit), 5);
        }

        this.handleMouseClick(
            (Slot) null, -999, Container.func_94534_d(2, this.dragSplittingLimit), 5);
  ...
  private boolean isMouseOverSlot(Slot slotIn, int mouseX, int mouseY) {
    return this.isPointInRegion(
        slotIn.xDisplayPosition, slotIn.yDisplayPosition, 16, 16, mouseX, mouseY);
  }
  ...
  protected boolean isPointInRegion(
      int left, int top, int right, int bottom, int pointX, int pointY) {
    int i = this.guiLeft;
    int j = this.guiTop;
    pointX = pointX - i;
    pointY = pointY - j;
    return pointX >= left - 1
        && pointX < left + right + 1
        && pointY >= top - 1
        && pointY < top + bottom + 1;
  }
  ...
  protected void handleMouseClick(Slot slotIn, int slotId, int clickedButton, int clickType) {
    ...
    this.mc.playerController.windowClick(
        this.inventorySlots.windowId, slotId, clickedButton, clickType, this.mc.thePlayer);
  }
  ...
  /**
   * This function is what controls the hotbar shortcut check when you press a number key when
   * hovering a stack. Args : keyCode, Returns true if a Hotbar key is pressed, else false
   */
