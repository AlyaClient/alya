...
public class GuiContainerCreative extends InventoryEffectRenderer {
  ...
  protected void handleMouseClick(Slot slotIn, int slotId, int clickedButton, int clickType) {
    ...
    if (slotIn == null
      ...
      if (slotIn == this.field_147064_C) {
        ...
        ItemStack itemstack =
            slotIn.decrStackSize(clickedButton == 0 ? 1 : slotIn.getStack().getMaxStackSize());
        ...
        this.mc.thePlayer.dropPlayerItemWithRandomChoice(
            this.mc.thePlayer.inventory.getItemStack(), true);
      ...
      this.inventorySlots.slotClick(
          slotIn == null ? slotId : slotIn.slotNumber, clickedButton, clickType, this.mc.thePlayer);

      if (Container.getDragEvent(clickedButton) == 2) {
        for (int i = 0; i < 9; ++i) {
          this.mc.playerController.sendSlotPacket(
              this.inventorySlots.getSlot(45 + i).getStack(), 36 + i);
        }
      } else if (slotIn != null) {
        ItemStack itemstack4 = this.inventorySlots.getSlot(slotIn.slotNumber).getStack();
        this.mc.playerController.sendSlotPacket(
            itemstack4, slotIn.slotNumber - this.inventorySlots.inventorySlots.size() + 9 + 36);
      }
    }
  }
  ...
  private void updateCreativeSearch() {
    ContainerCreative guicontainercreative$containercreative =
        (ContainerCreative) this.inventorySlots;
    guicontainercreative$containercreative.itemList.clear();

    for (Item item : Item.itemRegistry) {
      if (item != null && item.getCreativeTab() != null) {
        item.getSubItems(
            item, (CreativeTabs) null, guicontainercreative$containercreative.itemList);
      }
    }
  ...
  /**
   * returns (if you are not on the inventoryTab) and (the flag isn't set) and (you have more than 1
   * page of items)
   */
  ...
  private void setCurrentCreativeTab(CreativeTabs p_147050_1_) {
    ...
    if (p_147050_1_ == CreativeTabs.tabInventory) {
      ...
      for (int j = 0; j < container.inventorySlots.size(); ++j) {
        Slot slot =
            new CreativeSlot((Slot) container.inventorySlots.get(j), j);
  ...
  public void handleMouseInput() throws IOException {
    ...
    if (i != 0 && this.needsScrollBars()) {
      int j =
          ((ContainerCreative) this.inventorySlots).itemList.size() / 9 - 5;
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    if (this.field_147064_C != null
        ...
        && this.isPointInRegion(
            this.field_147064_C.xDisplayPosition,
            this.field_147064_C.yDisplayPosition,
            16,
            16,
            mouseX,
            mouseY)) {
      this.drawCreativeTabHoveringText(
          I18n.format("inventory.binSlot", new Object[0]), mouseX, mouseY);
    }
      ...
      if (creativetabs == null && stack.getItem() == Items.enchanted_book) {
        ...
        if (map.size() == 1) {
          Enchantment enchantment =
              Enchantment.getEnchantmentById(((Integer) map.keySet().iterator().next()).intValue());
      ...
      for (int k = 0; k < 5; ++k) {
        for (int l = 0; l < 9; ++l) {
          ...
          if (i1 >= 0 && i1 < this.itemList.size()) {
            GuiContainerCreative.field_147060_v.setInventorySlotContents(
                l + k * 9, (ItemStack) this.itemList.get(i1));
          } else {
            GuiContainerCreative.field_147060_v.setInventorySlotContents(
                l + k * 9, (ItemStack) null);
          }
        }
      }
    }
