...
public class GuiNewChat extends Gui {
          ...
          if (chatline != null) {
            ...
            if (j1 < 200 || flag) {
              ...
              if (l1 > 3) {
                ...
                boolean smoothChatEnabled = false;
                Optional<Module> hudModuleOpt =
                    Alya.getInstance().getModuleManager().getModule("HUD");
                if (hudModuleOpt.isPresent() && hudModuleOpt.get() instanceof HUDModule) {
                  HUDModule hudModule = (HUDModule) hudModuleOpt.get();
                  smoothChatEnabled = hudModule.isEnabled() && hudModule.getSmoothChatEnabled();
                }

                if (smoothChatEnabled) {
                  int animationDuration = 6;
                  if (j1 < animationDuration) {
                    float progress = (float) j1 / (float) animationDuration;
                    progress = 1.0F - (1.0F - progress) * (1.0F - progress);
                    animationOffsetY = (1.0F - progress) * 8.0F;
                  }
                }

                drawRect(
                    i2,
                    (int) (j2 - 9 + animationOffsetY),
                    i2 + l + 4,
                    (int) (j2 + animationOffsetY),
                    l1 / 2 << 24);
                String s = chatline.getChatComponent().getFormattedText();
                GlStateManager.enableBlend();
                font.drawStringWithShadow(
                    s, (float) i2, (float) (j2 - 8) + animationOffsetY, 16777215 + (l1 << 24));
  ...
  private void setChatLine(
      IChatComponent p_146237_1_, int p_146237_2_, int p_146237_3_, boolean p_146237_4_) {
    if (p_146237_2_ != 0) {
    ...
    List<IChatComponent> list =
        GuiUtilRenderComponents.func_178908_a(
            p_146237_1_, i, this.mc.fontRendererObj, false, false);
    ...
    if (!this.getChatOpen()) {
      ...
      if (j >= 0 && k >= 0) {
        ...
        if (j <= MathHelper.floor_float((float) this.getChatWidth() / this.getChatScale())
            && k < (int) font.getFontHeight() * l + l) {
          int i1 = k / (int) font.getFontHeight() + this.scrollPos;
                    ...
                    (int)
                        font.getStringWidth(
                            GuiUtilRenderComponents.func_178909_a(
                                ((ChatComponentText) ichatcomponent)
  ...
  public int getChatHeight() {
    return calculateChatboxHeight(
        this.getChatOpen()
            ? this.mc.gameSettings.chatHeightFocused
