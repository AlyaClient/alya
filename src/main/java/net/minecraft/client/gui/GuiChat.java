...
public class GuiChat extends GuiScreen {
  ...
  /**
   * keeps position of which chat message you will select when you press up, (does not increase for
   * duplicated messages sent immediately after each other)
   */
  ...
  protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    ...
    if (keystrokesOpt.isPresent() && keystrokesOpt.get() instanceof KeystrokesModule) {
      ...
      if (keystrokes.isEnabled() && keystrokes.mouseClicked(mouseX, mouseY, mouseButton)) {
        return;
      }
    }
  ...
  protected void mouseReleased(int mouseX, int mouseY, int state) {
    ...
    if (keystrokesOpt.isPresent() && keystrokesOpt.get() instanceof KeystrokesModule) {
      ...
      if (keystrokes.isEnabled()) {
        keystrokes.mouseReleased(mouseX, mouseY, state);
      }
    }
   ...
   * @param shouldOverwrite Determines if the text currently in the chat should be overwritten or
   *     appended
   */
  ...
  private void autocompleteCommand() {
    if (this.playerNamesFound) {
      ...
      String currentText = this.inputField.getText();
      List<String> completions = Alya.getInstance().getCommandManager().getCompletions(currentText);

      if (completions.isEmpty()) {
        return;
      }

      this.foundPlayerNames.addAll(completions);
      this.playerNamesFound = true;
    }
    ...
    if (this.foundPlayerNames.size() > 1) {
      ...
      for (String completion : this.foundPlayerNames) {
        if (stringbuilder.length() > 0) {
          stringbuilder.append(", ");
        }
  ...
  /**
   * input is relative and is applied directly to the sentHistoryCursor so -1 is the previous
   * message, 1 is the next message from the current cursor position
   *
   * @param msgPos The position of the message in the sent chat history relative to the current
   *     message.
   */
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    if (Mouse.isButtonDown(0)) {
      Optional<Module> keystrokesOpt =
          Alya.getInstance().getModuleManager().getModule("Keystrokes");
      if (keystrokesOpt.isPresent() && keystrokesOpt.get() instanceof KeystrokesModule) {
        KeystrokesModule keystrokes = (KeystrokesModule) keystrokesOpt.get();
        if (keystrokes.isEnabled() && keystrokes.isDragging()) {
          keystrokes.mouseDragged(mouseX, mouseY);
        }
      }
    }
