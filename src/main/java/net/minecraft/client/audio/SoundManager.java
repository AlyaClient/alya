...
public class SoundManager {
  ...
  public SoundManager(SoundHandler p_i45119_1_, GameSettings p_i45119_2_) {
    ...
    try {
      ...
      logger.error(
          LOG_MARKER,
          (String) "Error linking with the LibraryJavaSound plug-in",
  ...
  /**
   * Tries to add the paulscode library and the relevant codecs. If it fails, the master volume will
   * be set to zero.
   */
  private synchronized void loadSoundSystem() {
    if (!this.loaded) {
      try {
        (new Thread(
                new Runnable() {
                  public void run() {
                    SoundSystemConfig.setLogger(
                        new SoundSystemLogger() {
                          ...
                          public void importantMessage(
                              String p_importantMessage_1_, int p_importantMessage_2_) {
                          ...
                          public void errorMessage(
                              String p_errorMessage_1_,
                              String p_errorMessage_2_,
                              int p_errorMessage_3_) {
                            if (!p_errorMessage_2_.isEmpty()) {
                              SoundManager.logger.error(
                                  "Error in class \'" + p_errorMessage_1_ + "\'");
                    ...
                    SoundManager.this.sndSystem.setMasterVolume(
                        SoundManager.this.options.getSoundLevel(SoundCategory.MASTER));
        ...
        logger.error(
            LOG_MARKER,
            (String) "Error starting SoundSystem. Turning off sounds & music",
  ...
  /**
   * Returns the sound level (between 0.0 and 1.0) for a category, but 1.0 for the master sound
   * category
   */
  private float getSoundCategoryVolume(SoundCategory category) {
    return category != null && category != SoundCategory.MASTER
        ? this.options.getSoundLevel(category)
        : 1.0F;
  }
        ...
        for (String s : this.categorySounds.get(category)) {
          ...
          float f =
              this.getNormalizedVolume(
                  isound, (SoundPoolEntry) this.playingSoundPoolEntries.get(isound), category);
    ...
    for (ITickableSound itickablesound : this.tickableSounds) {
      ...
      if (itickablesound.isDonePlaying()) {
        ...
        this.sndSystem.setPitch(
            s,
            this.getNormalizedPitch(
                itickablesound, (SoundPoolEntry) this.playingSoundPoolEntries.get(itickablesound)));
        this.sndSystem.setPosition(
            s, itickablesound.getXPosF(), itickablesound.getYPosF(), itickablesound.getZPosF());
      }
    }
        ...
        if (i <= this.playTime) {
          ...
          try {
            this.categorySounds.remove(
                this.sndHandler.getSound(isound.getSoundLocation()).getSoundCategory(), s1);
  ...
  public boolean isSoundPlaying(ISound sound) {
    if (!this.loaded) {
      ...
      return s == null
          ? false
          : this.sndSystem.playing(s)
              || this.playingSoundsStopTime.containsKey(s)
    ...
    if (this.loaded) {
      if (this.sndSystem.getMasterVolume() <= 0.0F) {
        ...
        SoundEventAccessorComposite soundeventaccessorcomposite =
            this.sndHandler.getSound(sound.getSoundLocation());
  ...
  private float getNormalizedPitch(ISound sound, SoundPoolEntry entry) {
    return (float)
        MathHelper.clamp_double((double) sound.getPitch() * entry.getPitch(), 0.5D, 2.0D);
  }
  ...
  private float getNormalizedVolume(ISound sound, SoundPoolEntry entry, SoundCategory category) {
    return (float)
            MathHelper.clamp_double((double) sound.getVolume() * entry.getVolume(), 0.0D, 1.0D)
    ...
    if (this.loaded && player != null) {
      float f =
          player.prevRotationPitch
              + (player.rotationPitch - player.prevRotationPitch) * p_148615_2_;
      float f1 =
          player.prevRotationYaw + (player.rotationYaw - player.prevRotationYaw) * p_148615_2_;
