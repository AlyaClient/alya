...
public class ModelDragon extends ModelBase {
  ...
  public void render(
    ...
    float f =
        entitydragon.prevAnimTime
            + (entitydragon.animTime - entitydragon.prevAnimTime) * this.partialTicks;
    ...
    float f7 =
        this.updateRotations(
            entitydragon.getMovementOffsets(5, this.partialTicks)[0] + (double) (f6 / 2.0F));
    f2 = f2 + 2.0F;
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      this.spine.rotateAngleY =
          this.updateRotations(adouble1[0] - adouble[0]) * (float) Math.PI / 180.0F * f5;
      this.spine.rotateAngleX =
          f9 + (float) (adouble1[1] - adouble[1]) * (float) Math.PI / 180.0F * f5 * 5.0F;
      this.spine.rotateAngleZ =
          -this.updateRotations(adouble1[0] - (double) f7) * (float) Math.PI / 180.0F * f5;
      ...
      f3 =
          (float)
              ((double) f3
                  - Math.cos((double) this.spine.rotateAngleY)
                      * Math.cos((double) this.spine.rotateAngleX)
                      * 10.0D);
      f4 =
          (float)
              ((double) f4
                  - Math.sin((double) this.spine.rotateAngleY)
                      * Math.cos((double) this.spine.rotateAngleX)
                      * 10.0D);
    ...
    this.head.rotateAngleY =
        this.updateRotations(adouble2[0] - adouble[0]) * (float) Math.PI / 180.0F * 1.0F;
    this.head.rotateAngleZ =
        -this.updateRotations(adouble2[0] - (double) f7) * (float) Math.PI / 180.0F * 1.0F;
    ...
    for (int k = 0; k < 12; ++k) {
      ...
      f10 =
          (float)
              ((double) f10 + Math.sin((double) ((float) k * 0.45F + f8)) * 0.05000000074505806D);
      this.spine.rotateAngleY =
          (this.updateRotations(adouble2[0] - adouble[0]) * f5 + 180.0F) * (float) Math.PI / 180.0F;
      this.spine.rotateAngleX =
          f10 + (float) (adouble2[1] - adouble[1]) * (float) Math.PI / 180.0F * f5 * 5.0F;
      this.spine.rotateAngleZ =
          this.updateRotations(adouble2[0] - (double) f7) * (float) Math.PI / 180.0F * f5;
      this.spine.rotationPointY = f2;
      this.spine.rotationPointZ = f3;
      this.spine.rotationPointX = f4;
      f2 = (float) ((double) f2 + Math.sin((double) this.spine.rotateAngleX) * 10.0D);
  ...
  /**
   * Updates the rotations in the parameters for rotations greater than 180 degrees or less than
   * -180 degrees. It adds or subtracts 360 degrees, so that the appearance is the same, although
   * the numbers are then simplified to range -180 to 180
   */
