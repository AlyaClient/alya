...
public class EntityPlayerSP extends AbstractClientPlayer {
  ...
  public EntityPlayerSP(
      Minecraft mcIn, World worldIn, NetHandlerPlayClient netHandler, StatFileWriter statFile) {
  ...
  public void onUpdate() {
    if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
      Alya.getInstance().getEventBus().dispatch(new UpdateEvent());
  ...
  /**
   * called every tick when the player is on foot. Performs all the things that normally happen
   * during movement.
   */
  public void onUpdateWalkingPlayer() {
    ...
    if (flag != serverSprintState) {
      if (flag) {
        this.sendQueue.addToSendQueue(
            new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.START_SPRINTING));
      } else {
        this.sendQueue.addToSendQueue(
            new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.STOP_SPRINTING));
      }
    ...
    if (flag1 != this.serverSneakState) {
      if (flag1) {
        this.sendQueue.addToSendQueue(
            new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.START_SNEAKING));
      } else {
        this.sendQueue.addToSendQueue(
            new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.STOP_SNEAKING));
      }
      ...
      if (!motionEvent.isCanceled()) {
        ...
        if (this.ridingEntity == null) {
          if (flag2 && flag3) {
            ...
            this.sendQueue.addToSendQueue(
                new C03PacketPlayer.C04PacketPlayerPosition(posX, posY, posZ, onGround));
          } else if (flag3) {
            this.sendQueue.addToSendQueue(
          ...
          this.sendQueue.addToSendQueue(
              new C03PacketPlayer.C06PacketPlayerPosLook(
                  this.motionX, -999.0D, this.motionZ, rotationYaw, rotationPitch, onGround));
          flag2 = false;
        }
  ...
  public EntityItem dropOneItem(boolean dropAll) {
    ...
    this.sendQueue.addToSendQueue(
        new C07PacketPlayerDigging(
            c07packetplayerdigging$action, BlockPos.ORIGIN, EnumFacing.DOWN));
    return null;
  }
  ...
  public void sendChatMessage(String message) {
    if (message.startsWith(".")) {
      if (!Alya.getInstance().getCommandManager().executeCommand(message)) {
        this.addChatMessage(
            new ChatComponentText("§cUnknown command. Type .help for a list of commands."));
      }
    } else {
  ...
  public void sendHorseInventory() {
    this.sendQueue.addToSendQueue(
        new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.OPEN_INVENTORY));
  }
  ...
  public void updateEntityActionState() {
    ...
    if (this.isCurrentViewEntity()) {
      ...
      this.renderArmPitch =
          (float)
              ((double) this.renderArmPitch
                  + (double) (this.rotationPitch - this.renderArmPitch) * 0.5D);
      this.renderArmYaw =
          (float)
              ((double) this.renderArmYaw + (double) (this.rotationYaw - this.renderArmYaw) * 0.5D);
    }
  }
  ...
  public void onLivingUpdate() {
    ...
    this.pushOutOfBlocks(
        ...
        this.getEntityBoundingBox().minY + 0.5D,
