...
public class ThreadDownloadImageData extends SimpleTexture {
  ...
  public ThreadDownloadImageData(
      File cacheFileIn,
      String imageUrlIn,
  ...
  protected void loadTextureFromServer() {
    this.imageThread =
        new Thread("Texture Downloader #" + threadDownloadCounter.incrementAndGet()) {
          public void run() {
            ...
            if (ThreadDownloadImageData.this.shouldPipeline()) {
              ...
              try {
                httpurlconnection =
                    (HttpURLConnection)
                        (new URL(ThreadDownloadImageData.this.imageUrl))
                ...
                if (ThreadDownloadImageData.this.cacheFile != null) {
                  FileUtils.copyInputStreamToFile(
                      httpurlconnection.getInputStream(), ThreadDownloadImageData.this.cacheFile);
                ...
                if (ThreadDownloadImageData.this.imageBuffer != null) {
                  bufferedimage =
                      ThreadDownloadImageData.this.imageBuffer.parseUserSkin(bufferedimage);
                }
  ...
  private boolean shouldPipeline() {
    if (!this.pipeline) {
      ...
      return proxy.type() != Type.DIRECT && proxy.type() != Type.SOCKS
          ? false
  ...
  private void loadPipelined() {
    try {
      HttpRequest httprequest =
          HttpPipeline.makeRequest(this.imageUrl, Minecraft.getMinecraft().getProxy());
