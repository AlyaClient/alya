...
public class RenderSlime extends RenderLiving<EntitySlime> {
  ...
  public void doRender(
      EntitySlime entity, double x, double y, double z, float entityYaw, float partialTicks) {
  ...
  protected void preRenderCallback(EntitySlime entitylivingbaseIn, float partialTickTime) {
    ...
    float f1 =
        (entitylivingbaseIn.prevSquishFactor
                + (entitylivingbaseIn.squishFactor - entitylivingbaseIn.prevSquishFactor)
                    * partialTickTime)
            / (f * 0.5F + 1.0F);
