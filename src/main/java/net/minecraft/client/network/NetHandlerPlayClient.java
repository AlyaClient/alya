...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
   ...
   * handlePlayerPosLook to update positioning and handleJoinGame to inform the server of the client
   * distribution/mods)
   */
  ...
  private final Map<UUID, NetworkPlayerInfo> playerInfoMap =
      Maps.<UUID, NetworkPlayerInfo>newHashMap();
  ...
  public NetHandlerPlayClient(
      Minecraft mcIn, GuiScreen p_i46300_2_, NetworkManager p_i46300_3_, GameProfile p_i46300_4_) {
  ...
  public void handleSpawnExperienceOrb(S11PacketSpawnExperienceOrb packetIn) {
    ...
    Entity entity =
        new EntityXPOrb(
            this.clientWorldController,
            (double) packetIn.getX() / 32.0D,
            (double) packetIn.getY() / 32.0D,
            (double) packetIn.getZ() / 32.0D,
            packetIn.getXPValue());
    entity.serverPosX = packetIn.getX();
    entity.serverPosY = packetIn.getY();
    entity.serverPosZ = packetIn.getZ();
  ...
  /**
   * Invoked when the server registers new proximate objects in your watchlist or when objects in
   * your watchlist have changed -> Registers any changes locally
   */
  ...
  public void handleSpawnPlayer(S0CPacketSpawnPlayer packetIn) {
    ...
    entityotherplayermp.prevPosX =
        entityotherplayermp.lastTickPosX =
            (double) (entityotherplayermp.serverPosX = packetIn.getX());
    entityotherplayermp.prevPosY =
        entityotherplayermp.lastTickPosY =
            (double) (entityotherplayermp.serverPosY = packetIn.getY());
    entityotherplayermp.prevPosZ =
        entityotherplayermp.lastTickPosZ =
            (double) (entityotherplayermp.serverPosZ = packetIn.getZ());
    int i = packetIn.getCurrentItemID();

    if (i == 0) {
      entityotherplayermp.inventory.mainInventory[entityotherplayermp.inventory.currentItem] = null;
    } else {
      entityotherplayermp.inventory.mainInventory[entityotherplayermp.inventory.currentItem] =
   ...
   * the player is no longer registered as required to monitor them. The latter happens when
   * distance between the player and item increases beyond a certain treshold (typically the viewing
   * distance)
   */
   ...
   * only one block requires an update, the server sends S23PacketBlockChange and if 64 or more
   * blocks are changed, the server sends S21PacketChunkData
   */
  public void handleMultiBlockChange(S22PacketMultiBlockChange packetIn) {
    PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);

    for (S22PacketMultiBlockChange.BlockUpdateData s22packetmultiblockchange$blockupdatedata :
        packetIn.getChangedBlocks()) {
      this.clientWorldController.invalidateRegionAndSetBlock(
          s22packetmultiblockchange$blockupdatedata.getPos(),
  ...
  public void handleChunkData(S21PacketChunkData packetIn) {
    ...
    Chunk chunk =
        this.clientWorldController.getChunkFromChunkCoords(
            packetIn.getChunkX(), packetIn.getChunkZ());
    chunk.fillChunk(
        packetIn.func_149272_d(), packetIn.getExtractedSize(), packetIn.func_149274_i());
  ...
  public void handleCollectItem(S0DPacketCollectItem packetIn) {
    ...
    EntityLivingBase entitylivingbase =
        (EntityLivingBase) this.clientWorldController.getEntityByID(packetIn.getEntityID());
    ...
    if (entity != null) {
      if (entity instanceof EntityXPOrb) {
        this.clientWorldController.playSoundAtEntity(
            entity,
            "random.orb",
            0.2F,
      ...
      this.gameController.effectRenderer.addEffect(
          new EntityPickupFX(this.clientWorldController, entity, entitylivingbase, 0.5F));
  ...
  public void handleAnimation(S0BPacketAnimation packetIn) {
    ...
    if (entity != null) {
      if (packetIn.getAnimationType() == 0) {
        ...
        this.gameController.effectRenderer.emitParticleAtEntity(
            entity, EnumParticleTypes.CRIT_MAGIC);
      }
    }
  }
  ...
  /**
   * Spawns the mob entity at the specified location, with the specified rotation, momentum and
   * type. Updates the entities Datawatchers with the entity metadata specified in the packet
   */
  public void handleSpawnMob(S0FPacketSpawnMob packetIn) {
    ...
    EntityLivingBase entitylivingbase =
        (EntityLivingBase)
            EntityList.createEntityByID(packetIn.getEntityType(), this.gameController.theWorld);
  ...
  public void handleTimeUpdate(S03PacketTimeUpdate packetIn) {
    PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
    final TimeUpdateEvent timeUpdateEvent = new TimeUpdateEvent(packetIn);

    if (!timeUpdateEvent.isCanceled()) {
    ...
    Alya.getInstance().getEventBus().dispatch(timeUpdateEvent);
  }
  ...
  /**
   * Invokes the entities' handleUpdateHealth method which is implemented in LivingBase
   * (hurt/death), MinecartMobSpawner (spawn delay), FireworkRocket & MinecartTNT (explosion),
   * IronGolem (throwing,...), Witch (spawn particles), Zombie (villager transformation), Animal
   * (breeding mode particles), Horse (breeding/smoke particles), Sheep (...), Tameable (...),
  ...
  public void handleSetExperience(S1FPacketSetExperience packetIn) {
    PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
    this.gameController.thePlayer.setXPStats(
        packetIn.func_149397_c(), packetIn.getTotalExperience(), packetIn.getLevel());
  }
  ...
  /**
   * Initiates a new explosion (sound, particles, drop spawn) for the affected blocks indicated by
   * the packet.
   */
  ...
  public void handleOpenWindow(S2DPacketOpenWindow packetIn) {
    ...
    if ("minecraft:container".equals(packetIn.getGuiId())) {
      entityplayersp.displayGUIChest(
          new InventoryBasic(packetIn.getWindowTitle(), packetIn.getSlotCount()));
      entityplayersp.openContainer.windowId = packetIn.getWindowId();
    } else if ("minecraft:villager".equals(packetIn.getGuiId())) {
      entityplayersp.displayVillagerTradeGui(
          new NpcMerchant(entityplayersp, packetIn.getWindowTitle()));
      entityplayersp.openContainer.windowId = packetIn.getWindowId();
    } else if ("EntityHorse".equals(packetIn.getGuiId())) {
      Entity entity = this.clientWorldController.getEntityByID(packetIn.getEntityId());

      if (entity instanceof EntityHorse) {
        entityplayersp.displayGUIHorse(
            (EntityHorse) entity,
            new AnimalChest(packetIn.getWindowTitle(), packetIn.getSlotCount()));
        entityplayersp.openContainer.windowId = packetIn.getWindowId();
      }
    } else if (!packetIn.hasSlots()) {
      entityplayersp.displayGui(
          new LocalBlockIntercommunication(packetIn.getGuiId(), packetIn.getWindowTitle()));
      entityplayersp.openContainer.windowId = packetIn.getWindowId();
    } else {
      ContainerLocalMenu containerlocalmenu =
          new ContainerLocalMenu(
              packetIn.getGuiId(), packetIn.getWindowTitle(), packetIn.getSlotCount());
    ...
    if (packetIn.func_149175_c() == -1) {
      ...
      if (this.gameController.currentScreen instanceof GuiContainerCreative) {
        GuiContainerCreative guicontainercreative =
            (GuiContainerCreative) this.gameController.currentScreen;
        flag =
            guicontainercreative.getSelectedTabIndex() != CreativeTabs.tabInventory.getTabIndex();
      }

      if (packetIn.func_149175_c() == 0
          && packetIn.func_149173_d() >= 36
          && packetIn.func_149173_d() < 45) {
        ItemStack itemstack =
            entityplayer.inventoryContainer.getSlot(packetIn.func_149173_d()).getStack();
        ...
        entityplayer.openContainer.putStackInSlot(
            packetIn.func_149173_d(), packetIn.func_149174_e());
      }
    }
  }
  ...
  public void handleConfirmTransaction(S32PacketConfirmTransaction packetIn) {
    ...
    } else if (entityplayer.openContainer != null
        && packetIn.getWindowId() == entityplayer.openContainer.windowId) {
  ...
  public void handleWindowProperty(S31PacketWindowProperty packetIn) {
    ...
    if (entityplayer.openContainer != null
        && entityplayer.openContainer.windowId == packetIn.getWindowId()) {
  ...
  public void handleBlockAction(S24PacketBlockAction packetIn) {
    PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
    this.gameController.theWorld.addBlockEvent(
  ...
  public void handleBlockBreakAnim(S25PacketBlockBreakAnim packetIn) {
    ...
    this.gameController.theWorld.sendBlockBreakProgress(
        packetIn.getBreakerId(), packetIn.getPosition(), packetIn.getProgress());
  }
  ...
  public void handleChangeGameState(S2BPacketChangeGameState packetIn) {
    ...
    if (i >= 0
        && i < S2BPacketChangeGameState.MESSAGE_NAMES.length
    ...
    if (i == 1) {
      ...
      this.clientWorldController.playSound(
          entityplayer.posX,
  ...
  public void handleEffect(S28PacketEffect packetIn) {
    ...
    if (packetIn.isSoundServerwide()) {
      this.gameController.theWorld.playBroadcastSound(
          packetIn.getSoundType(), packetIn.getSoundPos(), packetIn.getSoundData());
    } else {
      this.gameController.theWorld.playAuxSFX(
          packetIn.getSoundType(), packetIn.getSoundPos(), packetIn.getSoundData());
    }
  }
    ...
    for (Entry<StatBase, Integer> entry : packetIn.func_148974_c().entrySet()) {
      ...
      if (statbase.isAchievement() && i > 0) {
        if (this.field_147308_k
            && this.gameController.thePlayer.getStatFileWriter().readStat(statbase) == 0) {
      ...
      this.gameController
          .thePlayer
          .getStatFileWriter()
          .unlockAchievement(this.gameController.thePlayer, statbase, i);
    }

    if (!this.field_147308_k
        && !flag
        && this.gameController.gameSettings.showInventoryAchievementHint) {
      this.gameController.guiAchievement.displayUnformattedAchievement(
  ...
  public void handleTitle(S45PacketTitle packetIn) {
    ...
    this.gameController.ingameGUI.displayTitle(
        s, s1, packetIn.getFadeInTime(), packetIn.getDisplayTime(), packetIn.getFadeOutTime());
  }
  ...
  public void handlePlayerListItem(S38PacketPlayerListItem packetIn) {
    ...
    for (S38PacketPlayerListItem.AddPlayerData s38packetplayerlistitem$addplayerdata :
        packetIn.func_179767_a()) {
      if (packetIn.func_179768_b() == S38PacketPlayerListItem.Action.REMOVE_PLAYER) {
        ...
        NetworkPlayerInfo networkplayerinfo =
            (NetworkPlayerInfo)
                this.playerInfoMap.get(s38packetplayerlistitem$addplayerdata.getProfile().getId());
        ...
        if (networkplayerinfo != null) {
          switch (packetIn.func_179768_b()) {
            ...
            case UPDATE_DISPLAY_NAME:
              networkplayerinfo.setDisplayName(
                  s38packetplayerlistitem$addplayerdata.getDisplayName());
          }
        }
      }
    }
  ...
  public void handleResourcePack(S48PacketResourcePackSend packetIn) {
    ...
    if (s.startsWith("level://")) {
      ...
      if (this.gameController.getCurrentServerData() != null
          && this.gameController.getCurrentServerData().getResourceMode()
        ...
        this.gameController.addScheduledTask(
            new Runnable() {
              public void run() {
                NetHandlerPlayClient.this.gameController.displayGuiScreen(
                    new GuiYesNo(
                        new GuiYesNoCallback() {
                          public void confirmClicked(boolean result, int id) {
                            ...
                            if (result) {
                              if (NetHandlerPlayClient.this.gameController.getCurrentServerData()
                                  != null) {
                              ...
                              NetHandlerPlayClient.this.netManager.sendPacket(
                                  new C19PacketResourcePackStatus(
                                      s1, C19PacketResourcePackStatus.Action.ACCEPTED));
                              Futures.addCallback(
                                  NetHandlerPlayClient.this
                                      .gameController
                              ...
                              NetHandlerPlayClient.this.netManager.sendPacket(
                                  new C19PacketResourcePackStatus(
                                      s1, C19PacketResourcePackStatus.Action.DECLINED));
                            }
                        ...
                        I18n.format("multiplayer.texturePrompt.line2", new Object[0]),
                        0));
              }
            });
      }
   ...
   * "MC|TrList" to acquire a MerchantRecipeList trades for a villager merchant, "MC|Brand" which
   * sets the server brand? on the player instance and finally "MC|RPack" which the server uses to
  ...
  public void handleCustomPayload(S3FPacketCustomPayload packetIn) {
    ...
    if ("MC|TrList".equals(packetIn.getChannelName())) {
      ...
      this.gameController.thePlayer.setClientBrand(
          packetIn.getBufferData().readStringFromBuffer(32767));
  ...
  public void handleScoreboardObjective(S3BPacketScoreboardObjective packetIn) {
    ...
    if (packetIn.func_149338_e() == 0) {
      ScoreObjective scoreobjective =
          scoreboard.addScoreObjective(packetIn.func_149339_c(), IScoreObjectiveCriteria.DUMMY);
   ...
   * the player-team- memberships, Set team displayname/prefix/suffix and/or whether friendly fire
   * is enabled
   */
  ...
  /**
   * Updates en entity's attributes and their respective modifiers, which are used for speed
   * bonusses (player sprinting, animals fleeing, baby speed), weapon/tool attackDamage, hostiles
   * followRange randomization, zombie maxHealth and knockback resistance as well as reinforcement
   * spawning chance.
   */
  public void handleEntityProperties(S20PacketEntityProperties packetIn) {
    ...
    if (entity != null) {
      if (!(entity instanceof EntityLivingBase)) {
        ...
        for (S20PacketEntityProperties.Snapshot s20packetentityproperties$snapshot :
            packetIn.func_149441_d()) {
          IAttributeInstance iattributeinstance =
              baseattributemap.getAttributeInstanceByName(
                  s20packetentityproperties$snapshot.func_151409_a());

          if (iattributeinstance == null) {
            iattributeinstance =
                baseattributemap.registerAttribute(
                    new RangedAttribute(
                        (IAttribute) null,
                        s20packetentityproperties$snapshot.func_151409_a(),
                        0.0D,
          ...
          for (AttributeModifier attributemodifier :
              s20packetentityproperties$snapshot.func_151408_c()) {
