...
import bypass.Alya;
import bypass.event.events.ChatReceivedEvent;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handleDestroyEntities(S13PacketDestroyEntities packetIn) {
        ...
        for(int i = 0;
            i < packetIn.getEntityIDs().length;
            ++i) {
    ...
    public void handleChat(S02PacketChat packetIn) {
        ...
        if(packetIn.getType() == 2) {
            ...
            final ChatReceivedEvent chatReceivedEvent = new ChatReceivedEvent(packetIn.getChatComponent());
            Alya.getInstance().getEventBus().dispatch(chatReceivedEvent);

            if(chatReceivedEvent.isCanceled()) {
                return;
            }

            this.gameController.ingameGUI.getChatGUI().printChatMessage(chatReceivedEvent.getMessage());
        }
    }
    ...
    public void handleTeams(S3EPacketTeams packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        if(this.clientWorldController == null || this.clientWorldController.getScoreboard() == null) {
            return;
        }
        Scoreboard scoreboard = this.clientWorldController.getScoreboard();
