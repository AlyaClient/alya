...
public class ShaderManager {
  ...
  private final Map<String, ShaderUniform> mappedShaderUniforms =
      Maps.<String, ShaderUniform>newHashMap();
  ...
  public ShaderManager(IResourceManager resourceManager, String programName)
    ...
    ResourceLocation resourcelocation =
        new ResourceLocation("shaders/program/" + programName + ".json");
    ...
    try {
      ...
      JsonObject jsonobject =
          jsonparser.parse(IOUtils.toString(inputstream, Charsets.UTF_8)).getAsJsonObject();
      ...
      this.field_148016_p =
          JsonBlendingMode.func_148110_a(
              JsonUtils.getJsonObject(jsonobject, "blend", (JsonObject) null));
      this.useFaceCulling = JsonUtils.getBoolean(jsonobject, "cull", true);
      this.vertexShaderLoader =
          ShaderLoader.loadShader(resourceManager, ShaderLoader.ShaderType.VERTEX, s);
      this.fragmentShaderLoader =
          ShaderLoader.loadShader(resourceManager, ShaderLoader.ShaderType.FRAGMENT, s1);
      ...
      if (this.shaderSamplers.get(this.samplerNames.get(i)) != null) {
        ...
        if (j != -1) {
          ...
          OpenGlHelper.glUniform1i(
              OpenGlHelper.glGetUniformLocation(
                  this.program, (CharSequence) this.samplerNames.get(i)),
              i);
        }
      }
    }
  ...
  public ShaderUniform getShaderUniformOrDefault(String p_147984_1_) {
    return (ShaderUniform)
        (this.mappedShaderUniforms.containsKey(p_147984_1_)
  ...
  private void setupUniforms() {
    ...
    for (ShaderUniform shaderuniform : this.shaderUniforms) {
      ...
      if (l == -1) {
        logger.warn(
            "Could not find uniform named " + s1 + " in the specified" + " shader program.");
      } else {
  ...
  private void parseUniform(JsonElement p_147987_1_) throws JsonException {
    ...
    if (jsonarray.size() != j && jsonarray.size() > 1) {
      throw new JsonException(
          "Invalid amount of values specified (expected "
              + j
              + ", found "
              + jsonarray.size()
              + ")");
