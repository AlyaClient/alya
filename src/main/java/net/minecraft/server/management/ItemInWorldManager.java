...
public class ItemInWorldManager {
  ...
  /**
   * Set to true when the "finished destroying block" packet is received but the block wasn't fully
   * damaged yet. The block will not be destroyed while this is false.
   */
    ...
    this.thisPlayerMP
        ...
        .sendPacketToAllPlayers(
            new S38PacketPlayerListItem(
                S38PacketPlayerListItem.Action.UPDATE_GAME_MODE,
  ...
  public void updateBlockRemoving() {
    ...
    if (this.receivedFinishDiggingPacket) {
      ...
      if (block.getMaterial() == Material.air) {
        ...
        float f =
            block.getPlayerRelativeBlockHardness(
                    this.thisPlayerMP, this.thisPlayerMP.worldObj, this.field_180241_i)
                * (float) (i + 1);
        int j = (int) (f * 10.0F);

        if (j != this.durabilityRemainingOnBlock) {
          this.theWorld.sendBlockBreakProgress(
              this.thisPlayerMP.getEntityId(), this.field_180241_i, j);
      ...
      if (block1.getMaterial() == Material.air) {
        this.theWorld.sendBlockBreakProgress(
            this.thisPlayerMP.getEntityId(), this.field_180240_f, -1);
        ...
        if (l != this.durabilityRemainingOnBlock) {
          this.theWorld.sendBlockBreakProgress(
              this.thisPlayerMP.getEntityId(), this.field_180240_f, l);
  ...
  public void onBlockClicked(BlockPos pos, EnumFacing side) {
    if (this.isCreative()) {
      ...
      if (block.getMaterial() != Material.air) {
        ...
        f =
            block.getPlayerRelativeBlockHardness(
                this.thisPlayerMP, this.thisPlayerMP.worldObj, pos);
      }
  ...
  public void blockRemoving(BlockPos pos) {
    if (pos.equals(this.field_180240_f)) {
      ...
      if (block.getMaterial() != Material.air) {
        float f =
            block.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.worldObj, pos)
                * (float) (i + 1);
    ...
    if (this.gameType.isCreative()
      ...
      if (this.isCreative()) {
        this.thisPlayerMP.playerNetServerHandler.sendPacket(
            new S23PacketBlockChange(this.theWorld, pos));
      } else {
        ...
        if (itemstack1 != null) {
          itemstack1.onBlockDestroyed(
              this.theWorld, iblockstate.getBlock(), pos, this.thisPlayerMP);
        ...
        if (flag1 && flag) {
          iblockstate
              .getBlock()
              .harvestBlock(this.theWorld, this.thisPlayerMP, pos, iblockstate, tileentity);
        }
      }
  ...
  public boolean activateBlockOrUseItem(
    ...
    if (this.gameType == WorldSettings.GameType.SPECTATOR) {
      ...
      if (!player.isSneaking() || player.getHeldItem() == null) {
        ...
        if (iblockstate
            .getBlock()
            .onBlockActivated(worldIn, pos, iblockstate, player, side, offsetX, offsetY, offsetZ)) {
          return true;
        }
      }
