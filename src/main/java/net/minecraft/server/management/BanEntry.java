...
public abstract class BanEntry<T> extends UserListEntry<T> {
  ...
  protected BanEntry(T p_i1174_1_, JsonObject p_i1174_2_) {
    ...
    try {
      date =
          p_i1174_2_.has("created")
              ? dateFormat.parse(p_i1174_2_.get("created").getAsString())
              : new Date();
    ...
    try {
      date1 =
          p_i1174_2_.has("expires")
              ? dateFormat.parse(p_i1174_2_.get("expires").getAsString())
              : null;
    ...
    this.reason =
        p_i1174_2_.has("reason")
            ? p_i1174_2_.get("reason").getAsString()
  ...
  protected void onSerialization(JsonObject data) {
    ...
    data.addProperty(
        "expires", this.banEndDate == null ? "forever" : dateFormat.format(this.banEndDate));
