...
public class PlayerProfileCache {
  ...
  private final Map<String, PlayerProfileCache.ProfileEntry> usernameToProfileEntryMap =
      Maps.<String, PlayerProfileCache.ProfileEntry>newHashMap();
  private final Map<UUID, PlayerProfileCache.ProfileEntry> uuidToProfileEntryMap =
      Maps.<UUID, PlayerProfileCache.ProfileEntry>newHashMap();
  ...
  public PlayerProfileCache(MinecraftServer server, File cacheFile) {
    ...
    gsonbuilder.registerTypeHierarchyAdapter(
        PlayerProfileCache.ProfileEntry.class, new PlayerProfileCache.Serializer());
   ...
   *
   * <p>The UUID of the GameProfile will <b>not</b> be null. If the server is offline, a UUID based
   * on the hash of the username will be used.
   *
    ...
    ProfileLookupCallback profilelookupcallback =
        new ProfileLookupCallback() {
          ...
          public void onProfileLookupFailed(
              GameProfile p_onProfileLookupFailed_1_, Exception p_onProfileLookupFailed_2_) {
            agameprofile[0] = null;
          }
        };
    server
        .getGameProfileRepository()
        .findProfilesByNames(new String[] {username}, Agent.MINECRAFT, profilelookupcallback);
   ...
   *
   * @param gameProfile The entry's {@link GameProfile}
   * @param expirationDate The expiration date for this entry. {@code null} is allowed, 1 month will
  ...
  private void addEntry(GameProfile gameProfile, Date expirationDate) {
    ...
    PlayerProfileCache.ProfileEntry playerprofilecache$profileentry =
        new PlayerProfileCache.ProfileEntry(gameProfile, expirationDate);

    if (this.uuidToProfileEntryMap.containsKey(uuid)) {
      PlayerProfileCache.ProfileEntry playerprofilecache$profileentry1 =
          (PlayerProfileCache.ProfileEntry) this.uuidToProfileEntryMap.get(uuid);
      this.usernameToProfileEntryMap.remove(
          playerprofilecache$profileentry1.getGameProfile().getName().toLowerCase(Locale.ROOT));
      this.gameProfiles.remove(gameProfile);
    }

    this.usernameToProfileEntryMap.put(
        gameProfile.getName().toLowerCase(Locale.ROOT), playerprofilecache$profileentry);
  ...
  /**
   * Get a player's GameProfile given their username. Mojang's server's will be contacted if the
   * entry is not cached locally.
   *
   * @param username The player's username
   */
  public GameProfile getGameProfileForUsername(String username) {
    ...
    if (playerprofilecache$profileentry != null
      ...
      this.usernameToProfileEntryMap.remove(
          playerprofilecache$profileentry.getGameProfile().getName().toLowerCase(Locale.ROOT));
    ...
    this.save();
    return playerprofilecache$profileentry == null
        ? null
  ...
  public void load() {
    ...
    try {
      ...
      List<PlayerProfileCache.ProfileEntry> list =
          (List) this.gson.fromJson((Reader) bufferedreader, TYPE);
  ...
  private List<PlayerProfileCache.ProfileEntry> getEntriesWithLimit(int limitSize) {
    ArrayList<PlayerProfileCache.ProfileEntry> arraylist =
        Lists.<PlayerProfileCache.ProfileEntry>newArrayList();

    for (GameProfile gameprofile :
        Lists.newArrayList(Iterators.limit(this.gameProfiles.iterator(), limitSize))) {
      PlayerProfileCache.ProfileEntry playerprofilecache$profileentry =
          this.getByUUID(gameprofile.getId());
  ...
  class Serializer
    ...
    public JsonElement serialize(
      ...
      jsonobject.addProperty(
          "expiresOn", PlayerProfileCache.dateFormat.format(p_serialize_1_.getExpirationDate()));
      return jsonobject;
    }

    public PlayerProfileCache.ProfileEntry deserialize(
      ...
      if (p_deserialize_1_.isJsonObject()) {
        ...
        if (jsonelement != null && jsonelement1 != null) {
          ...
          if (s1 != null && s != null) {
            ...
            PlayerProfileCache.ProfileEntry playerprofilecache$profileentry =
                PlayerProfileCache.this.new ProfileEntry(new GameProfile(uuid, s1), date);
            return playerprofilecache$profileentry;
          } else {
            return null;
          }
