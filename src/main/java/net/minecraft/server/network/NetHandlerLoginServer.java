...
public class NetHandlerLoginServer implements INetHandlerLoginServer, ITickable {
  ...
  private NetHandlerLoginServer.LoginState currentLoginState =
      NetHandlerLoginServer.LoginState.HELLO;
  ...
  public void update() {
    if (this.currentLoginState == NetHandlerLoginServer.LoginState.READY_TO_ACCEPT) {
      ...
      EntityPlayerMP entityplayermp =
          this.server.getConfigurationManager().getPlayerByUUID(this.loginGameProfile.getId());

      if (entityplayermp == null) {
        this.currentLoginState = NetHandlerLoginServer.LoginState.READY_TO_ACCEPT;
        this.server
            .getConfigurationManager()
            .initializeConnectionToPlayer(this.networkManager, this.field_181025_l);
  ...
  public void tryAcceptPlayer() {
    ...
    if (s != null) {
      ...
      if (this.server.getNetworkCompressionTreshold() >= 0
          && !this.networkManager.isLocalChannel()) {
  ...
  public String getConnectionInfo() {
    return this.loginGameProfile != null
        ? this.loginGameProfile.toString()
            + " ("
            + this.networkManager.getRemoteAddress().toString()
            + ")"
  ...
  public void processLoginStart(C00PacketLoginStart packetIn) {
    ...
    if (this.server.isServerInOnlineMode() && !this.networkManager.isLocalChannel()) {
      ...
      this.networkManager.sendPacket(
          new S01PacketEncryptionRequest(
              this.serverId, this.server.getKeyPair().getPublic(), this.verifyToken));
    } else {
      this.currentLoginState = NetHandlerLoginServer.LoginState.READY_TO_ACCEPT;
    }
  }
              ...
              try {
                ...
                NetHandlerLoginServer.this.loginGameProfile =
                    NetHandlerLoginServer.this
                        ...
                        .hasJoinedServer(
                            new GameProfile((UUID) null, gameprofile.getName()), s, null);
                ...
                if (NetHandlerLoginServer.this.loginGameProfile != null) {
                  ...
                  NetHandlerLoginServer.this.currentLoginState =
                      NetHandlerLoginServer.LoginState.READY_TO_ACCEPT;
                } else if (NetHandlerLoginServer.this.server.isSinglePlayer()) {
                  NetHandlerLoginServer.logger.warn(
                      "Failed to verify username but will let them in anyway!");
                  NetHandlerLoginServer.this.loginGameProfile =
                      NetHandlerLoginServer.this.getOfflineProfile(gameprofile);
                  NetHandlerLoginServer.this.currentLoginState =
                      NetHandlerLoginServer.LoginState.READY_TO_ACCEPT;
                } else {
                ...
                if (NetHandlerLoginServer.this.server.isSinglePlayer()) {
                  ...
                  NetHandlerLoginServer.this.closeConnection(
