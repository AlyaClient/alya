...
public class ItemArmorStand extends Item {
          ...
          if (list.size() > 0) {
            ...
            if (!worldIn.isRemote) {
              ...
              EntityArmorStand entityarmorstand =
                  new EntityArmorStand(worldIn, d0 + 0.5D, d1, d2 + 0.5D);
              float f =
                  (float)
                          MathHelper.floor_float(
                              (MathHelper.wrapAngleTo180_float(playerIn.rotationYaw - 180.0F)
                                      + 22.5F)
                                  / 45.0F)
                      * 45.0F;
  ...
  private void applyRandomRotations(EntityArmorStand armorStand, Random rand) {
    ...
    Rotations rotations1 =
        new Rotations(rotations.getX() + f, rotations.getY() + f1, rotations.getZ());
