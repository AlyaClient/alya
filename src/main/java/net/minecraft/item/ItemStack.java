...
public final class ItemStack {
  ...
  /**
   * Called when the player uses this ItemStack on a Block (right-click). Places blocks, etc.
   * (Legacy name: tryPlaceItemIntoWorld)
   */
  ...
  /**
   * Called whenever this item stack is equipped and right clicked. Returns the new item stack to
   * put in the position where this item is. Args: world, player
   */
  ...
  /**
   * Called when the item in use count reach 0, e.g. item food eaten. Return the new ItemStack. Args
   * : world, entity
   */
  ...
  public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
    ResourceLocation resourcelocation =
        (ResourceLocation) Item.itemRegistry.getNameForObject(this.item);
  ...
  /**
   * Attempts to damage the ItemStack with par1 amount of damage, If the ItemStack has the
   * Unbreaking enchantment there is a chance for each point of damage to be negated. Returns true
   * if it takes more damage than getMaxDamage(). Returns false otherwise or if the ItemStack can't
   * be damaged or if all points of damage are negated.
   */
        ...
        if (this.attemptDamageItem(amount, entityIn.getRNG())) {
          ...
          if (entityIn instanceof EntityPlayer) {
            ...
            entityplayer.triggerAchievement(
                StatList.objectBreakStats[Item.getIdFromItem(this.item)]);
  ...
  /**
   * compares ItemStack argument1 with ItemStack argument2; returns true if both ItemStacks are
   * equal
   */
  ...
  /**
   * compares ItemStack argument to the instance ItemStack; returns true if both ItemStacks are
   * equal
   */
  ...
  /**
   * compares ItemStack argument to the instance ItemStack; returns true if the Items contained in
  ...
  /**
   * Called each tick as long the ItemStack in on player inventory. Used to progress the pickup
   * animation and update maps.
   */
  public void updateAnimation(
      World worldIn, Entity entityIn, int inventorySlot, boolean isCurrentItem) {
  ...
  /**
   * Assigns a NBTTagCompound to the ItemStack, minecraft validates that only non-stackable items
   * can have it.
   */
    ...
    if (advanced) {
      ...
      if (this.hasTagCompound()) {
        list.add(
            EnumChatFormatting.DARK_GRAY
                + "NBT: "
                + this.getTagCompound().getKeySet().size()
                + " tag(s)");
      }
    }
    ...
    if (this.hasTagCompound() && this.stackTagCompound.hasKey("AttributeModifiers", 9)) {
      ...
      for (int i = 0; i < nbttaglist.tagCount(); ++i) {
        ...
        AttributeModifier attributemodifier =
            SharedMonsterAttributes.readAttributeModifierFromNBT(nbttagcompound);
  ...
  public IChatComponent getChatComponent() {
    ...
    IChatComponent ichatcomponent =
        (new ChatComponentText("[")).appendSibling(chatcomponenttext).appendText("]");
