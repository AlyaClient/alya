...
public class ItemMap extends ItemMapBase {
    ...
    public MapData getMapData(ItemStack stack, World worldIn) {
        ...
        if(mapdata == null && !worldIn.isRemote) {
            ...
            mapdata.calculateMapCenter(
                    (double) worldIn.getWorldInfo().getSpawnX(),
                    (double) worldIn.getWorldInfo().getSpawnZ(),
                    mapdata.scale);
                    ...
                    for(int l1 = i1 - j1 - 1; l1 < i1 + j1; ++l1) {
                        if(k1 >= 0 && l1 >= -1 && k1 < 128 && l1 < 128) {
                            ...
                            if(!chunk.isEmpty()) {
                                            ...
                                            if(k4 > 1) {
                                                label541:
                                                {
                                                    while(true) {
                                                        ...
                                                        iblockstate =
                                                                chunk.getBlockState(
                                                                        blockpos$mutableblockpos.func_181079_c(i4 + i3, k4, j4 + j3));

                                                        if(iblockstate.getBlock().getMapColor(iblockstate) != MapColor.airColor
                                                                || k4 <= 0) {
                                                            break;
                                                        }
                                                    }
                                ...
                                k3 = k3 / (i * i);
                                double d2 =
                                        (d1 - d0) * 4.0D / (double) (i + 4) + ((double) (k1 + l1 & 1) - 0.5D) * 0.4D;
                                int i5 = 1;
    ...
    public void onCreated(ItemStack stack, World worldIn, EntityPlayer playerIn) {
        if(stack.hasTagCompound() && stack.getTagCompound().getBoolean("map_is_scaling")) {
            ...
            mapdata1.calculateMapCenter(
                    (double) mapdata.xCenter, (double) mapdata.zCenter, mapdata1.scale);
