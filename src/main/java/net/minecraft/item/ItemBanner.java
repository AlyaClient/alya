...
public class ItemBanner extends ItemBlock {
    ...
    if (side == EnumFacing.DOWN) {
      ...
      if (!playerIn.canPlayerEdit(pos, side, stack)) {
        ...
        if (side == EnumFacing.UP) {
          ...
          worldIn.setBlockState(
              pos,
              Blocks.wall_banner.getDefaultState().withProperty(BlockWallSign.FACING, side),
              3);
        }
    ...
    if (nbttagcompound != null && nbttagcompound.hasKey("Patterns")) {
      ...
      for (int i = 0; i < nbttaglist.tagCount() && i < 6; ++i) {
        ...
        TileEntityBanner.EnumBannerPattern tileentitybanner$enumbannerpattern =
            TileEntityBanner.EnumBannerPattern.getPatternByID(nbttagcompound1.getString("Pattern"));
  ...
  public void getSubItems(Item itemIn, CreativeTabs tab, List<ItemStack> subItems) {
    for (EnumDyeColor enumdyecolor : EnumDyeColor.values()) {
      ...
      TileEntityBanner.func_181020_a(
          nbttagcompound, enumdyecolor.getDyeDamage(), (NBTTagList) null);
