...
public abstract class MobSpawnerBaseLogic {
  ...
  private final List<MobSpawnerBaseLogic.WeightedRandomMinecart> minecartToSpawn =
      Lists.<MobSpawnerBaseLogic.WeightedRandomMinecart>newArrayList();
    ...
    if (this.isActivated()) {
      ...
      if (this.getSpawnerWorld().isRemote) {
        ...
        this.getSpawnerWorld()
            .spawnParticle(
                EnumParticleTypes.SMOKE_NORMAL, d3, d4, d5, 0.0D, 0.0D, 0.0D, new int[0]);
        this.getSpawnerWorld()
            .spawnParticle(EnumParticleTypes.FLAME, d3, d4, d5, 0.0D, 0.0D, 0.0D, new int[0]);
        ...
        this.mobRotation =
            (this.mobRotation + (double) (1000.0F / ((float) this.spawnDelay + 200.0F))) % 360.0D;
      } else {
        ...
        for (int i = 0; i < this.spawnCount; ++i) {
          ...
          entity.setLocationAndAngles(
              d0, d1, d2, this.getSpawnerWorld().rand.nextFloat() * 360.0F, 0.0F);
  ...
  private Entity spawnNewEntity(Entity entityIn, boolean spawn) {
    if (this.getRandomEntity() != null) {
      ...
      for (Entity entity = entityIn;
          nbttagcompound.hasKey("Riding", 10);
          nbttagcompound = nbttagcompound2) {
        nbttagcompound2 = nbttagcompound.getCompoundTag("Riding");
        Entity entity1 =
            EntityList.createEntityByName(nbttagcompound2.getString("id"), entityIn.worldObj);
      ...
      if (entityIn instanceof EntityLiving) {
        ((EntityLiving) entityIn)
            .onInitialSpawn(
                entityIn.worldObj.getDifficultyForLocation(new BlockPos(entityIn)),
    ...
    if (nbt.hasKey("SpawnPotentials", 9)) {
      ...
      for (int i = 0; i < nbttaglist.tagCount(); ++i) {
        this.minecartToSpawn.add(
            new MobSpawnerBaseLogic.WeightedRandomMinecart(nbttaglist.getCompoundTagAt(i)));
      }
    }
    ...
    if (nbt.hasKey("SpawnData", 10)) {
      this.setRandomEntity(
          new MobSpawnerBaseLogic.WeightedRandomMinecart(
              nbt.getCompoundTag("SpawnData"), this.mobID));
    } else {
  ...
  public void writeToNBT(NBTTagCompound nbt) {
    ...
    if (!StringUtils.isNullOrEmpty(s)) {
      ...
      if (this.getRandomEntity() != null || this.minecartToSpawn.size() > 0) {
        ...
        if (this.minecartToSpawn.size() > 0) {
          for (MobSpawnerBaseLogic.WeightedRandomMinecart
              mobspawnerbaselogic$weightedrandomminecart : this.minecartToSpawn) {
  ...
  public class WeightedRandomMinecart extends WeightedRandom.Item {
    ...
    public WeightedRandomMinecart(NBTTagCompound tagCompound) {
      this(
          tagCompound.getCompoundTag("Properties"),
          tagCompound.getString("Type"),
    ...
    private WeightedRandomMinecart(NBTTagCompound tagCompound, String type, int weight) {
      ...
      if (type.equals("Minecart")) {
        if (tagCompound != null) {
          type =
              EntityMinecart.EnumMinecartType.byNetworkID(tagCompound.getInteger("Type")).getName();
        } else {
