...
public class TileEntityBrewingStand extends TileEntityLockable
        ...
        for (int i = 0; i < BlockBrewingStand.HAS_BOTTLE.length; ++i) {
          iblockstate =
              iblockstate.withProperty(
                  BlockBrewingStand.HAS_BOTTLE[i], Boolean.valueOf(aboolean[i]));
        }
      ...
      if (!itemstack.getItem().isPotionIngredient(itemstack)) {
        ...
        for (int i = 0; i < 3; ++i) {
          if (this.brewingItemStacks[i] != null
            ...
            if ((j <= 0 || list != list1)
                && (list == null || !list.equals(list1) && list1 != null)
                && j != k) {
              flag = true;
              break;
            }
  ...
  public ItemStack getStackInSlot(int index) {
    return index >= 0 && index < this.brewingItemStacks.length
        ? this.brewingItemStacks[index]
        : null;
  }
