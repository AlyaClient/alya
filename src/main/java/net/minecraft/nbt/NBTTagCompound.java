...
public class NBTTagCompound extends NBTBase {
  ...
  public float getFloat(String key) {
    try {
      return !this.hasKey(key, 99)
          ? 0.0F
          : ((NBTBase.NBTPrimitive) this.tagMap.get(key)).getFloat();
    } catch (ClassCastException var3) {
      return 0.0F;
    }
  }
  ...
  public double getDouble(String key) {
    try {
      return !this.hasKey(key, 99)
          ? 0.0D
          : ((NBTBase.NBTPrimitive) this.tagMap.get(key)).getDouble();
    } catch (ClassCastException var3) {
      return 0.0D;
    }
  }
  ...
  public int[] getIntArray(String key) {
    try {
      return !this.hasKey(key, 11)
          ? new int[0]
          : ((NBTTagIntArray) this.tagMap.get(key)).getIntArray();
    } catch (ClassCastException classcastexception) {
  ...
  public NBTTagList getTagList(String key, int type) {
    try {
      if (this.getTagId(key) != 9) {
        ...
        return nbttaglist.tagCount() > 0 && nbttaglist.getTagType() != type
            ? new NBTTagList()
            : nbttaglist;
      }
    } catch (ClassCastException classcastexception) {
  ...
  private CrashReport createCrashReport(
      final String key, final int expectedType, ClassCastException ex) {
  ...
  static NBTBase readNBT(
      byte id, String key, DataInput input, int depth, NBTSizeTracker sizeTracker)
      throws IOException {
