...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO {
  ...
  private Set<ChunkCoordIntPair> pendingAnvilChunksCoordinates =
      Collections.<ChunkCoordIntPair>newSetFromMap(new ConcurrentHashMap());
  ...
  public Chunk loadChunk(World worldIn, int x, int z) throws IOException {
    ...
    if (nbttagcompound == null) {
      DataInputStream datainputstream =
          RegionFileCache.getChunkInputStream(this.chunkSaveLocation, x, z);
  ...
  public boolean writeNextIO() {
    if (this.chunksToRemove.isEmpty()) {
      ...
      ChunkCoordIntPair chunkcoordintpair =
          (ChunkCoordIntPair) this.chunksToRemove.keySet().iterator().next();
      boolean lvt_3_1_;

      try {
        this.pendingAnvilChunksCoordinates.add(chunkcoordintpair);
        NBTTagCompound nbttagcompound =
            (NBTTagCompound) this.chunksToRemove.remove(chunkcoordintpair);
  ...
  private void func_183013_b(ChunkCoordIntPair p_183013_1_, NBTTagCompound p_183013_2_)
    ...
    DataOutputStream dataoutputstream =
        RegionFileCache.getChunkOutputStream(
            this.chunkSaveLocation, p_183013_1_.chunkXPos, p_183013_1_.chunkZPos);
    CompressedStreamTools.write(p_183013_2_, dataoutputstream);
    dataoutputstream.close();
  }

  /**
   * Save extra data associated with this Chunk not normally saved during autosave, only during
   * chunk unload. Currently unused.
   */
  ...
  /**
   * Writes the Chunk passed as an argument to the NBTTagCompound also passed, using the World
   * argument to retrieve the Chunk's last update time.
   */
    ...
    for (ExtendedBlockStorage extendedblockstorage : aextendedblockstorage) {
      if (extendedblockstorage != null) {
        ...
        nbttagcompound.setByteArray(
            "BlockLight", extendedblockstorage.getBlocklightArray().getData());

        if (flag) {
          nbttagcompound.setByteArray(
              "SkyLight", extendedblockstorage.getSkylightArray().getData());
        } else {
          nbttagcompound.setByteArray(
              "SkyLight", new byte[extendedblockstorage.getBlocklightArray().getData().length]);
        }
    ...
    if (list != null) {
      ...
      for (NextTickListEntry nextticklistentry : list) {
        ...
        ResourceLocation resourcelocation =
            (ResourceLocation) Block.blockRegistry.getNameForObject(nextticklistentry.getBlock());
  ...
  /**
   * Reads the data stored in the passed NBTTagCompound and creates a Chunk with that data in the
   * passed World. Returns the created Chunk.
   */
      ...
      for (int k2 = 0; k2 < nbttaglist1.tagCount(); ++k2) {
        ...
        if (entity != null) {
          ...
          for (NBTTagCompound nbttagcompound4 = nbttagcompound1;
            ...
            Entity entity2 =
                EntityList.createEntityFromNBT(nbttagcompound4.getCompoundTag("Riding"), worldIn);
