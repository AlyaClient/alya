...
public class Chunk {
  ...
  /**
   * Used to store block IDs, block MSBs, Sky-light maps, Block-light maps, and metadata. Each entry
   * corresponds to a logical segment of 16x16x16 blocks, stacked vertically.
   */
  ...
  /**
   * Contains the current round-robin relight check index, and is implied as the relight check
   * location as well.
   */
  ...
  /**
   * Returns the topmost ExtendedBlockStorage instance for this Chunk that actually contains a
   * block.
   */
  ...
  /**
   * Propagates a given sky-visible block's light value downward and upward to neighboring blocks as
   * necessary.
   */
  ...
  private void recheckGaps(boolean p_150803_1_) {
    ...
    if (this.worldObj.isAreaLoaded(
        new BlockPos(this.xPosition * 16 + 8, 0, this.zPosition * 16 + 8), 16)) {
      for (int i = 0; i < 16; ++i) {
        for (int j = 0; j < 16; ++j) {
          if (this.updateSkylightColumns[i + j * 16]) {
              ...
              j1 =
                  Math.min(
                      ...
                      this.worldObj.getChunksLowestHorizon(
                          l + enumfacing.getFrontOffsetX(), i1 + enumfacing.getFrontOffsetZ()));
            }
            ...
            for (EnumFacing enumfacing1 : EnumFacing.Plane.HORIZONTAL) {
              this.checkSkylightNeighborHeight(
                  l + enumfacing1.getFrontOffsetX(), i1 + enumfacing1.getFrontOffsetZ(), k);
            }
  ...
  /**
   * Checks the height of a block next to a sky-visible block and schedules a lighting update as
   * necessary.
   */
      ...
      if (!this.worldObj.provider.getHasNoSky()) {
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
          this.updateSkylightNeighborHeight(
              k + enumfacing.getFrontOffsetX(), l + enumfacing.getFrontOffsetZ(), j2, k2);
        }
  ...
  public Block getBlock(final int x, final int y, final int z) {
    try {
      ...
      CrashReportCategory crashreportcategory =
          reportedexception.getCrashReport().makeCategory("Block being got");
  ...
  private int getBlockMetadata(int x, int y, int z) {
    if (y >> 4 >= this.storageArrays.length) {
      ...
      return extendedblockstorage != null
          ? extendedblockstorage.getExtBlockMetadata(x, y & 15, z)
          : 0;
    }
  }
    ...
    if (iblockstate == state) {
      ...
      if (extendedblockstorage == null) {
        ...
        extendedblockstorage =
            this.storageArrays[j >> 4] =
                new ExtendedBlockStorage(j >> 4 << 4, !this.worldObj.provider.getHasNoSky());
        flag = j >= i1;
      }
      ...
      if (extendedblockstorage.getBlockByExtId(i, j & 15, k) != block) {
        ...
        if (block instanceof ITileEntityProvider) {
          ...
          if (tileentity1 == null) {
            tileentity1 =
                ((ITileEntityProvider) block)
                    .createNewTileEntity(this.worldObj, block.getMetaFromState(state));
  ...
  private TileEntity createNewTileEntity(BlockPos pos) {
    ...
    return !block.hasTileEntity()
        ? null
        : ((ITileEntityProvider) block)
            .createNewTileEntity(this.worldObj, this.getBlockMetadata(pos));
  }
  ...
  public <T extends Entity> void getEntitiesOfTypeWithinAAAB(
    ...
    for (int k = i; k <= j; ++k) {
      for (T t : this.entityLists[k].getByClass(entityClass)) {
        if (t.getEntityBoundingBox().intersectsWith(aabb)
            && (p_177430_4_ == null || p_177430_4_.apply(t))) {
          listToFill.add(t);
        }
      }
    }
  ...
  public void populateChunk(
      IChunkProvider p_76624_1_, IChunkProvider p_76624_2_, int p_76624_3_, int p_76624_4_) {
  ...
  public void setStorageArrays(ExtendedBlockStorage[] newStorageArrays) {
    if (this.storageArrays.length != newStorageArrays.length) {
      logger.warn(
          "Could not set level chunk sections, array length is "
              + newStorageArrays.length
              + " instead of "
  ...
  public void fillChunk(byte[] p_177439_1_, int p_177439_2_, boolean p_177439_3_) {
      ...
      if ((p_177439_2_ & 1 << j) != 0) {
        ...
        for (int k = 0; k < achar.length; ++k) {
          if (i + 1 >= p_177439_1_.length) {
            logger.warn("Chunk data too small, skipping rest of chunk loading");
            return;
          }
    ...
    for (int l = 0; l < this.storageArrays.length; ++l) {
      if ((p_177439_2_ & 1 << l) != 0 && this.storageArrays[l] != null) {
        ...
        if (i + nibblearray.getData().length > p_177439_1_.length) {
          logger.warn("Chunk data too small for blocklight, skipping rest of chunk loading");
          return;
        }
    ...
    if (flag) {
      for (int i1 = 0; i1 < this.storageArrays.length; ++i1) {
        if ((p_177439_2_ & 1 << i1) != 0 && this.storageArrays[i1] != null) {
          ...
          if (i + nibblearray1.getData().length > p_177439_1_.length) {
            logger.warn("Chunk data too small for skylight, skipping rest of chunk loading");
            return;
          }
          System.arraycopy(
              p_177439_1_, i, nibblearray1.getData(), 0, nibblearray1.getData().length);
    ...
    if (p_177439_3_) {
      if (i + this.blockBiomeArray.length > p_177439_1_.length) {
        logger.warn("Chunk data too small for biome data, skipping rest of chunk loading");
        return;
      }
  ...
  /**
   * Returns an array containing a 16x16 mapping on the X/Z of block positions in this Chunk to
   * biome IDs.
   */
  ...
  /**
   * Accepts a 256-entry array that contains a 16x16 mapping on the X/Z plane of block positions in
   * this Chunk to biome IDs.
   */
  public void setBiomeArray(byte[] biomeArray) {
    if (this.blockBiomeArray.length != biomeArray.length) {
      logger.warn(
          "Could not set level chunk biomes, array length is "
              + biomeArray.length
              + " instead of "
  ...
  /**
   * Called once-per-chunk-per-tick, and advances the round-robin relight check index by up to 8
   * blocks at a time. In a worst-case scenario, can potentially take up to 25.6 seconds, calculated
   * via (4096/8)/20, to re-check all blocks in a chunk, which may explain lagging light updates on
  ...
  public void func_150809_p() {
    ...
    if (!this.worldObj.provider.getHasNoSky()) {
      if (this.worldObj.isAreaLoaded(
          blockpos.add(-1, 0, -1), blockpos.add(16, this.worldObj.func_181545_F(), 16))) {
        label92:
        for (int i = 0; i < 16; ++i) {
          for (int j = 0; j < 16; ++j) {
        ...
        if (this.isLightPopulated) {
          for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            this.worldObj
                .getChunkFromBlockCoords(blockpos.offset(enumfacing, k))
  ...
  private boolean func_150811_f(int x, int z) {
    ...
    BlockPos.MutableBlockPos blockpos$mutableblockpos =
        new BlockPos.MutableBlockPos((this.xPosition << 4) + x, 0, (this.zPosition << 4) + z);

    for (int j = i + 16 - 1; j > this.worldObj.func_181545_F() || j > 0 && !flag1; --j) {
      blockpos$mutableblockpos.func_181079_c(
          blockpos$mutableblockpos.getX(), j, blockpos$mutableblockpos.getZ());
    ...
    for (int l = blockpos$mutableblockpos.getY(); l > 0; --l) {
      blockpos$mutableblockpos.func_181079_c(
          blockpos$mutableblockpos.getX(), l, blockpos$mutableblockpos.getZ());
  ...
  public void setHeightMap(int[] newHeightMap) {
    if (this.heightMap.length != newHeightMap.length) {
      logger.warn(
          "Could not set level chunk heightmap, array length is "
              + newHeightMap.length
              + " instead of "
