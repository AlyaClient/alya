...
public abstract class World implements IBlockAccess {
   ...
   * 0000000000LLLLzzzzzzyyyyyyxxxxxx. The 4-bit L is a light level used when darkening blocks.
   * 6-bit numbers x, y and z represent the block's offset from the original block, plus 32 (i.e.
   * value of 31 would mean a -1 offset
   */
    ...
    if (this.isBlockLoaded(pos)) {
      ...
      try {
        ...
        CrashReportCategory crashreportcategory =
            crashreport.makeCategory("Coordinates of biome request");
  ...
  /**
   * Sets a new spawn location by finding an uncovered block at a random (x,z) location in the
   * chunk.
   */
  ...
  public Block getGroundAboveSeaLevel(BlockPos pos) {
    ...
    for (blockpos = new BlockPos(pos.getX(), this.func_181545_F(), pos.getZ());
        !this.isAirBlock(blockpos.up());
  ...
  /**
   * Checks to see if an air block exists at the provided location. Note that this only checks to
   * see if the blocks material is set to air, meaning it is possible for non-vanilla blocks to
  ...
  public boolean isBlockLoaded(BlockPos pos, boolean allowEmpty) {
    return !this.isValid(pos)
        ? false
        : this.isChunkLoaded(pos.getX() >> 4, pos.getZ() >> 4, allowEmpty);
  }
  ...
  public boolean isAreaLoaded(BlockPos from, BlockPos to, boolean allowEmpty) {
    return this.isAreaLoaded(
        from.getX(), from.getY(), from.getZ(), to.getX(), to.getY(), to.getZ(), allowEmpty);
  }
  ...
  public boolean isAreaLoaded(StructureBoundingBox box, boolean allowEmpty) {
    return this.isAreaLoaded(
        box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ, allowEmpty);
  }
  ...
  protected boolean isChunkLoaded(int x, int z, boolean allowEmpty) {
    return this.chunkProvider.chunkExists(x, z)
        && (allowEmpty || !this.chunkProvider.provideChunk(x, z).isEmpty());
  }
  ...
  /**
   * Sets the block state at a given location. Flag 1 will cause a block update. Flag 2 will send
   * the change to clients (you almost always want this). Flag 4 prevents the block from being
   * re-rendered, if this is a client world. Flags can be added together.
   *
   * @param flags Flag 1 will cause a block update. Flag 2 will send the change to clients (you
   *     almost always want this). Flag 4 prevents the block from being re-rendered, if this is a
  ...
  public void markBlockRangeForRenderUpdate(int x1, int y1, int z1, int x2, int y2, int z2) {
    for (int i = 0; i < this.worldAccesses.size(); ++i) {
      ((IWorldAccess) this.worldAccesses.get(i))
          .markBlockRangeForRenderUpdate(x1, y1, z1, x2, y2, z2);
    }
  }
    ...
    if (!this.isRemote) {
      ...
      try {
        ...
        CrashReport crashreport =
            CrashReport.makeCrashReport(throwable, "Exception while updating neighbours");
  ...
  /**
   * Returns the position at this x, z coordinate in the chunk with y set to the value from the
   * height map.
   */
  public BlockPos getHeight(BlockPos pos) {
    ...
    if (pos.getX() >= -30000000
      ...
      if (this.isChunkLoaded(pos.getX() >> 4, pos.getZ() >> 4, true)) {
        i =
            this.getChunkFromChunkCoords(pos.getX() >> 4, pos.getZ() >> 4)
  ...
  public MovingObjectPosition rayTraceBlocks(
    ...
    if (!Double.isNaN(vec31.xCoord) && !Double.isNaN(vec31.yCoord) && !Double.isNaN(vec31.zCoord)) {
      if (!Double.isNaN(vec32.xCoord)
          && !Double.isNaN(vec32.yCoord)
        ...
        if ((!ignoreBlockWithoutBoundingBox
          ...
          MovingObjectPosition movingobjectposition =
              block.collisionRayTrace(this, blockpos, vec31, vec32);
        ...
        while (k1-- >= 0) {
          if (Double.isNaN(vec31.xCoord)
              || Double.isNaN(vec31.yCoord)
          ...
          if (!ignoreBlockWithoutBoundingBox
            ...
            if (block1.canCollideCheck(iblockstate1, stopOnLiquid)) {
              MovingObjectPosition movingobjectposition1 =
                  block1.collisionRayTrace(this, blockpos, vec31, vec32);
  ...
  public void playSoundAtEntity(Entity entityIn, String name, float volume, float pitch) {
    for (int i = 0; i < this.worldAccesses.size(); ++i) {
      ((IWorldAccess) this.worldAccesses.get(i))
          .playSound(name, entityIn.posX, entityIn.posY, entityIn.posZ, volume, pitch);
    }
  }
    ...
    for (int i = 0; i < this.worldAccesses.size(); ++i) {
      ((IWorldAccess) this.worldAccesses.get(i))
          .playSoundToNearExcept(
              player, name, player.posX, player.posY, player.posZ, volume, pitch);
    }
  }

  /**
   * Play a sound effect. Many many parameters for this function. Not sure what they do, but a
   * classic call is : (double)i + 0.5D, (double)j + 0.5D, (double)k + 0.5D, 'random.door_open',
   * 1.0F, world.rand.nextFloat() * 0.1F + 0.9F with i,j,k position of the block.
   */
  ...
  public void spawnParticle(
    ...
    this.spawnParticle(
        particleType.getParticleID(),
        particleType.getShouldIgnoreRange() | p_175682_2_,
        xCoord,
        yCoord,
        zCoord,
        xOffset,
          ...
          for (int i2 = k - 1; i2 < l; ++i2) {
            ...
            iblockstate1
                ...
                .addCollisionBoxesToList(
                    this, blockpos$mutableblockpos, iblockstate1, bb, list, entityIn);
          }
        }
      }
    }
        ...
        if (this.isBlockLoaded(blockpos$mutableblockpos.func_181079_c(k1, 64, l1))) {
          for (int i2 = k - 1; i2 < l; ++i2) {
            ...
            iblockstate
                .getBlock()
                .addCollisionBoxesToList(
                    this, blockpos$mutableblockpos, iblockstate, bb, list, (Entity) null);
          }
        }
      }
    }
  ...
  public int calculateSkylightSubtracted(float p_72967_1_) {
    ...
    f1 = 1.0F - f1;
    f1 =
        (float) ((double) f1 * (1.0D - (double) (this.getRainStrength(p_72967_1_) * 5.0F) / 16.0D));
    f1 =
        (float)
            ((double) f1 * (1.0D - (double) (this.getThunderStrength(p_72967_1_) * 5.0F) / 16.0D));
    f1 = 1.0F - f1;
    return (int) (f1 * 11.0F);
  }
  ...
  public float getSunBrightness(float p_72971_1_) {
    ...
    f1 = 1.0F - f1;
    f1 =
        (float) ((double) f1 * (1.0D - (double) (this.getRainStrength(p_72971_1_) * 5.0F) / 16.0D));
    f1 =
        (float)
            ((double) f1 * (1.0D - (double) (this.getThunderStrength(p_72971_1_) * 5.0F) / 16.0D));
    return f1 * 0.8F + 0.2F;
  }
  ...
  public float getCurrentMoonPhaseFactor() {
    return WorldProvider.moonPhaseFactors[
      ...
      if (!entity2.isDead) {
        try {
          ...
          CrashReportCategory crashreportcategory2 =
              crashreport1.makeCategory("Entity being ticked");
        ...
        if (this.isBlockLoaded(blockpos) && this.worldBorder.contains(blockpos)) {
          try {
            ...
            CrashReportCategory crashreportcategory1 =
                crashreport2.makeCategory("Block entity being ticked");
  ...
  public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate) {
    ...
    if (!forceUpdate || this.isAreaLoaded(i - k, 0, j - k, i + k, 0, j + k, true)) {
      ...
      if (Double.isNaN((double) entityIn.rotationPitch)
          || Double.isInfinite((double) entityIn.rotationPitch)) {
        entityIn.rotationPitch = entityIn.prevRotationPitch;
      }

      if (Double.isNaN((double) entityIn.rotationYaw)
          || Double.isInfinite((double) entityIn.rotationYaw)) {
      ...
      if (!entityIn.addedToChunk
          || entityIn.chunkCoordX != l
          || entityIn.chunkCoordY != i1
          || entityIn.chunkCoordZ != j1) {
        if (entityIn.addedToChunk
            && this.isChunkLoaded(entityIn.chunkCoordX, entityIn.chunkCoordZ, true)) {
  ...
  public boolean checkNoEntityCollision(AxisAlignedBB bb, Entity entityIn) {
    ...
    for (int i = 0; i < list.size(); ++i) {
      ...
      if (!entity.isDead
          ...
          && entity != entityIn
          && (entityIn == null
              || entityIn.ridingEntity != entity && entityIn.riddenByEntity != entity)) {
        return false;
      }
    }
    ...
    for (int k1 = i; k1 <= j; ++k1) {
      for (int l1 = k; l1 <= l; ++l1) {
        for (int i2 = i1; i2 <= j1; ++i2) {
          Block block =
              this.getBlockState(blockpos$mutableblockpos.func_181079_c(k1, l1, i2)).getBlock();
  ...
  public boolean isMaterialInBB(AxisAlignedBB bb, Material materialIn) {
    ...
    for (int k1 = i; k1 < j; ++k1) {
      for (int l1 = k; l1 < l; ++l1) {
        for (int i2 = i1; i2 < j1; ++i2) {
          if (this.getBlockState(blockpos$mutableblockpos.func_181079_c(k1, l1, i2))
                  .getBlock()
                  .getMaterial()
              == materialIn) {
            return true;
  ...
  public boolean isAABBInMaterial(AxisAlignedBB bb, Material materialIn) {
    ...
    for (int k1 = i; k1 < j; ++k1) {
      for (int l1 = k; l1 < l; ++l1) {
        for (int i2 = i1; i2 < j1; ++i2) {
          IBlockState iblockstate =
              this.getBlockState(blockpos$mutableblockpos.func_181079_c(k1, l1, i2));
          Block block = iblockstate.getBlock();
  ...
  public boolean isBlockFullCube(BlockPos pos) {
    ...
    AxisAlignedBB axisalignedbb =
        iblockstate.getBlock().getCollisionBoundingBox(this, pos, iblockstate);
    ...
    if (this.ambientTickCountdown == 0 && !this.isRemote) {
      ...
      if (block.getMaterial() == Material.air
        ...
        EntityPlayer entityplayer =
            this.getClosestPlayer((double) j + 0.5D, (double) l + 0.5D, (double) k + 0.5D, 8.0D);

        if (entityplayer != null
            && entityplayer.getDistanceSq((double) j + 0.5D, (double) l + 0.5D, (double) k + 0.5D)
                > 4.0D) {
   ...
   * @param noWaterAdj If true, this method will only return true if there is a non-water block
   *     immediately adjacent to the specified block
   */
              ...
              if (j3 + k3 + l3 < 17) {
                ...
                for (EnumFacing enumfacing : EnumFacing.values()) {
                  ...
                  int l4 =
                      Math.max(
                          1,
                          this.getBlockState(blockpos$mutableblockpos)
                              .getBlock()
                              .getLightOpacity());
                  i3 = this.getLightFor(lightType, blockpos$mutableblockpos);

                  if (i3 == l2 - l4 && j < this.lightUpdateBlockList.length) {
                    this.lightUpdateBlockList[j++] =
                        i4 - i1 + 32 | j4 - j1 + 32 << 6 | k4 - k1 + 32 << 12 | l2 - l4 << 18;
                  }
                }
              }
            }
          ...
          if (j6 > i6) {
            ...
            if (k6 + l6 + i7 < 17 && flag) {
              if (this.getLightFor(lightType, blockpos1.west()) < j6) {
                this.lightUpdateBlockList[j++] =
                    j5 - 1 - i1 + 32 + (k5 - j1 + 32 << 6) + (l5 - k1 + 32 << 12);
              }
              ...
              if (this.getLightFor(lightType, blockpos1.down()) < j6) {
                this.lightUpdateBlockList[j++] =
                    j5 - i1 + 32 + (k5 - 1 - j1 + 32 << 6) + (l5 - k1 + 32 << 12);
              }
              ...
              if (this.getLightFor(lightType, blockpos1.north()) < j6) {
                this.lightUpdateBlockList[j++] =
                    j5 - i1 + 32 + (k5 - j1 + 32 << 6) + (l5 - 1 - k1 + 32 << 12);
              }
  ...
  public List<NextTickListEntry> func_175712_a(
      StructureBoundingBox structureBB, boolean p_175712_2_) {
    return null;
  }
  ...
  public List<Entity> getEntitiesInAABBexcluding(
      Entity entityIn, AxisAlignedBB boundingBox, Predicate<? super Entity> predicate) {
  ...
  public <T extends Entity> List<T> getEntities(
      Class<? extends T> entityType, Predicate<? super T> filter) {
  ...
  public <T extends Entity> List<T> getPlayers(
      Class<? extends T> playerType, Predicate<? super T> filter) {
    List<T> list = Lists.<T>newArrayList();
  ...
  public <T extends Entity> List<T> getEntitiesWithinAABB(
      Class<? extends T> clazz, AxisAlignedBB aabb, Predicate<? super T> filter) {
    ...
    for (int i1 = i; i1 <= j; ++i1) {
      for (int j1 = k; j1 <= l; ++j1) {
        if (this.isChunkLoaded(i1, j1, true)) {
          this.getChunkFromChunkCoords(i1, j1)
              .getEntitiesOfTypeWithinAAAB(clazz, aabb, list, filter);
        }
      }
    }
  ...
  public <T extends Entity> T findNearestEntityWithinAABB(
      Class<? extends T> entityType, AxisAlignedBB aabb, T closestTo) {
  ...
  public boolean canBlockBePlaced(
      Block blockIn,
      BlockPos pos,
      boolean p_175716_3_,
      EnumFacing side,
    ...
    return axisalignedbb != null && !this.checkNoEntityCollision(axisalignedbb, entityIn)
        ...
        : (block.getMaterial() == Material.circuits && blockIn == Blocks.anvil
            ...
            : block.getMaterial().isReplaceable()
                && blockIn.canReplace(this, pos, side, itemStackIn));
  }
  ...
  public int getRedstonePower(BlockPos pos, EnumFacing facing) {
    ...
    return block.isNormalCube()
        ...
        : block.isProvidingWeakPower(this, pos, iblockstate, facing);
  }
  ...
  /**
   * Checks if the specified block or its neighbors are powered by a neighboring block. Used by
   * blocks like TNT and Doors.
   */
  ...
  public BlockPos getSpawnPoint() {
    BlockPos blockpos =
        new BlockPos(
            this.worldInfo.getSpawnX(), this.worldInfo.getSpawnY(), this.worldInfo.getSpawnZ());

    if (!this.getWorldBorder().contains(blockpos)) {
      blockpos =
          this.getHeight(
              new BlockPos(
                  this.getWorldBorder().getCenterX(), 0.0D, this.getWorldBorder().getCenterZ()));
    }
  ...
  public float getThunderStrength(float delta) {
    return (this.prevThunderingStrength
            + (this.thunderingStrength - this.prevThunderingStrength) * delta)
  ...
  /**
   * Assigns the given String id to the given MapDataBase using the MapStorage, removing any
   ...
   * MapStorage, instantiating the given Class, or returns null if none such file exists. args:
  ...
  /**
   * Returns an unique new data id from the MapStorage for the given prefix and saves the idCounts
  ...
  public void playAuxSFXAtEntity(EntityPlayer player, int sfxType, BlockPos pos, int p_180498_4_) {
    try {
      ...
      CrashReportCategory crashreportcategory =
          crashreport.makeCategory("Level event being played");
      crashreportcategory.addCrashSection(
          "Block coordinates", CrashReportCategory.getCoordinateInfo(pos));
  ...
  public Random setRandomSeed(int p_72843_1_, int p_72843_2_, int p_72843_3_) {
    long i =
        (long) p_72843_1_ * 341873128712L
            + (long) p_72843_2_ * 132897987541L
  ...
  public CrashReportCategory addWorldInfoToCrashReport(CrashReport report) {
    ...
    crashreportcategory.addCrashSection(
        "Level name", this.worldInfo == null ? "????" : this.worldInfo.getWorldName());
    crashreportcategory.addCrashSectionCallable(
        "All players",
        new Callable<String>() {
          public String call() {
            return World.this.playerEntities.size()
                + " total; "
