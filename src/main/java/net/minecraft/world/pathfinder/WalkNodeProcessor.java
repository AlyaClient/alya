...
public class WalkNodeProcessor extends NodeProcessor {
  ...
  /**
   * This method is called when all nodes have been processed and PathEntity is created. {@link
   * net.minecraft.world.pathfinder.WalkNodeProcessor WalkNodeProcessor} uses this to change its
   * field {@link net.minecraft.world.pathfinder.WalkNodeProcessor#avoidsWater avoidsWater}
   */
  ...
  public int findPathOptions(
    ...
    if (this.getVerticalOffset(
            entityIn, currentPoint.xCoord, currentPoint.yCoord + 1, currentPoint.zCoord)
        == 1) {
      j = 1;
    }
    ...
    PathPoint pathpoint =
        this.getSafePoint(
            entityIn, currentPoint.xCoord, currentPoint.yCoord, currentPoint.zCoord + 1, j);
    PathPoint pathpoint1 =
        this.getSafePoint(
            entityIn, currentPoint.xCoord - 1, currentPoint.yCoord, currentPoint.zCoord, j);
    ...
    if (pathpoint1 != null
        && !pathpoint1.visited
        && pathpoint1.distanceTo(targetPoint) < maxDistance) {
      pathOptions[i++] = pathpoint1;
    }

    if (pathpoint2 != null
        && !pathpoint2.visited
        && pathpoint2.distanceTo(targetPoint) < maxDistance) {
      pathOptions[i++] = pathpoint2;
    }

    if (pathpoint3 != null
        && !pathpoint3.visited
        && pathpoint3.distanceTo(targetPoint) < maxDistance) {
  ...
  /**
   * Checks if an entity collides with blocks at a position. Returns 1 if clear, 0 for colliding
   * with any solid block, -1 for water(if avoids water), -2 for lava, -3 for fence and wall, -4 for
   * closed trapdoor, 2 if otherwise clear except for open trapdoor or water(if not avoiding)
   */
      ...
      for (int j = y; j < y + sizeY; ++j) {
        for (int k = z; k < z + sizeZ; ++k) {
          ...
          if (block.getMaterial() != Material.air) {
            ...
            if (entityIn.worldObj.getBlockState(blockpos$mutableblockpos).getBlock()
                instanceof BlockRailBase) {
              if (!(entityIn.worldObj.getBlockState(blockpos).getBlock() instanceof BlockRailBase)
                  && !(entityIn.worldObj.getBlockState(blockpos.down()).getBlock()
                      instanceof BlockRailBase)) {
                return -3;
              }
