...
public class MapData extends WorldSavedData {
  ...
  private Map<EntityPlayer, MapData.MapInfo> playersHashMap =
      Maps.<EntityPlayer, MapData.MapInfo>newHashMap();
  ...
  /**
   * Adds the player passed to the list of visible players and checks to see which players are
   * visible
   */
    ...
    for (int i = 0; i < this.playersArrayList.size(); ++i) {
      ...
      if (!mapdata$mapinfo1.entityplayerObj.isDead
        ...
        if (!mapStack.isOnItemFrame()
            && mapdata$mapinfo1.entityplayerObj.dimension == this.dimension) {
  ...
  private void updatePlayersVisibleOnMap(
      int type,
      World worldIn,
