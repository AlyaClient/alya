...
public class MapGenStronghold extends MapGenStructure {
  ...
  /**
   * is spawned false and set true once the defined BiomeGenBases were compared with the present
   * ones
   */
  ...
  public MapGenStronghold(Map<String, String> p_i2068_1_) {
    ...
    for (Entry<String, String> entry : p_i2068_1_.entrySet()) {
      if (((String) entry.getKey()).equals("distance")) {
        this.field_82671_h =
            MathHelper.parseDoubleWithDefaultAndMax(
                (String) entry.getValue(), this.field_82671_h, 1.0D);
      } else if (((String) entry.getKey()).equals("count")) {
        this.structureCoords =
            new ChunkCoordIntPair
                [MathHelper.parseIntWithDefaultAndMax(
                    (String) entry.getValue(), this.structureCoords.length, 1)];
      } else if (((String) entry.getKey()).equals("spread")) {
        this.field_82672_i =
            MathHelper.parseIntWithDefaultAndMax((String) entry.getValue(), this.field_82672_i, 1);
      }
    }
  }
      ...
      for (int j = 0; j < this.structureCoords.length; ++j) {
        ...
        BlockPos blockpos =
            this.worldObj
                ...
                .findBiomePosition((k << 4) + 8, (l << 4) + 8, 112, this.field_151546_e, random);
  ...
  protected StructureStart getStructureStart(int chunkX, int chunkZ) {
    ...
    for (mapgenstronghold$start =
            new MapGenStronghold.Start(this.worldObj, this.rand, chunkX, chunkZ);
        mapgenstronghold$start.getComponents().isEmpty()
            || ((StructureStrongholdPieces.Stairs2) mapgenstronghold$start.getComponents().get(0))
  ...
  public static class Start extends StructureStart {
    ...
    public Start(World worldIn, Random p_i2067_2_, int p_i2067_3_, int p_i2067_4_) {
      ...
      structurestrongholdpieces$stairs2.buildComponent(
          structurestrongholdpieces$stairs2, this.components, p_i2067_2_);
      ...
      while (!list.isEmpty()) {
        ...
        structurecomponent.buildComponent(
            structurestrongholdpieces$stairs2, this.components, p_i2067_2_);
      }
