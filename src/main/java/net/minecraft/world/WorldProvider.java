...
public abstract class WorldProvider {
  ...
  /**
   * States whether the Hell world provider is used(true) or if the normal world provider is
   * used(false)
   */
    ...
    if (worldtype == WorldType.FLAT) {
      FlatGeneratorInfo flatgeneratorinfo =
          FlatGeneratorInfo.createFlatGeneratorFromString(
              this.worldObj.getWorldInfo().getGeneratorOptions());
      this.worldChunkMgr =
          new WorldChunkManagerHell(
              BiomeGenBase.getBiomeFromBiomeList(
                  flatgeneratorinfo.getBiome(), BiomeGenBase.field_180279_ad),
              0.5F);
    ...
    return (WorldProvider)
        (dimension == -1
            ...
            : (dimension == 0
                ...
                : (dimension == 1 ? new WorldProviderEnd() : null)));
  }
  ...
  /**
   * Returns a double value representing the Y value relative to the top of the map at which void
   * fog is at its maximum. The default factor of 0.03125 relative to 256, for example, means the
   * void fog will be at its maximum at (256*0.03125), or 8.
   */
