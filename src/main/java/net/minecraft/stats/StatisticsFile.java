  ...
  public void readStatFile() {
    if (this.statsFile.isFile()) {
      try {
        ...
        logger.error(
            (String) ("Couldn\'t read statistics file " + this.statsFile), (Throwable) ioexception);
      } catch (JsonParseException jsonparseexception) {
        logger.error(
            (String) ("Couldn\'t parse statistics file " + this.statsFile),
  ...
  public Map<StatBase, TupleIntJsonSerializable> parseJson(String p_150881_1_) {
    ...
    if (!jsonelement.isJsonObject()) {
      ...
      Map<StatBase, TupleIntJsonSerializable> map =
          Maps.<StatBase, TupleIntJsonSerializable>newHashMap();
    ...
    for (Entry<StatBase, TupleIntJsonSerializable> entry : p_150880_0_.entrySet()) {
      if (((TupleIntJsonSerializable) entry.getValue()).getJsonSerializableValue() != null) {
        ...
        try {
          jsonobject1.add(
              "progress",
              ((TupleIntJsonSerializable) entry.getValue())
