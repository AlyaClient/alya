...
public class S26PacketMapChunkBulk implements Packet<INetHandlerPlayClient> {
  ...
  public S26PacketMapChunkBulk(List<Chunk> chunks) {
    ...
    for (int j = 0; j < i; ++j) {
      ...
      S21PacketChunkData.Extracted s21packetchunkdata$extracted =
          S21PacketChunkData.func_179756_a(chunk, true, this.isOverworld, 65535);
  ...
  public void readPacketData(PacketBuffer buf) throws IOException {
    ...
    for (int j = 0; j < i; ++j) {
      ...
      this.chunksData[j].data =
          new byte
              [S21PacketChunkData.func_180737_a(
                  Integer.bitCount(this.chunksData[j].dataSize), this.isOverworld, true)];
    }
