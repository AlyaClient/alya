...
public class S34PacketMaps implements Packet<INetHandlerPlayClient> {
  ...
  public S34PacketMaps(
      int mapIdIn,
      byte scale,
      Collection<Vec4b> visiblePlayers,
      byte[] colors,
      int minX,
      int minY,
      int maxX,
    ...
    this.mapId = mapIdIn;
    this.mapScale = scale;
    this.mapVisiblePlayersVec4b =
        (Vec4b[]) visiblePlayers.toArray(new Vec4b[visiblePlayers.size()]);
    this.mapMinX = minX;
    this.mapMinY = minY;
    this.mapMaxX = maxX;
    this.mapMaxY = maxY;
  ...
  public void readPacketData(PacketBuffer buf) throws IOException {
    ...
    for (int i = 0; i < this.mapVisiblePlayersVec4b.length; ++i) {
      ...
      this.mapVisiblePlayersVec4b[i] =
          new Vec4b(
              (byte) (short1 >> 4 & 15), buf.readByte(), buf.readByte(), (byte) (short1 & 15));
    }
  ...
  public void setMapdataTo(MapData mapdataIn) {
    ...
    for (int j = 0; j < this.mapMaxX; ++j) {
      for (int k = 0; k < this.mapMaxY; ++k) {
        mapdataIn.colors[this.mapMinX + j + (this.mapMinY + k) * 128] =
            this.mapDataBytes[j + k * this.mapMaxX];
      }
    }
  }
}
