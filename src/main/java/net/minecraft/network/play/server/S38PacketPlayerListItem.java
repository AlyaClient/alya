...
public class S38PacketPlayerListItem implements Packet<INetHandlerPlayClient> {
  ...
  private final List<S38PacketPlayerListItem.AddPlayerData> players =
      Lists.<S38PacketPlayerListItem.AddPlayerData>newArrayList();

  public S38PacketPlayerListItem() {}

  public S38PacketPlayerListItem(
      S38PacketPlayerListItem.Action actionIn, EntityPlayerMP... players) {
  ...
  public S38PacketPlayerListItem(
      S38PacketPlayerListItem.Action actionIn, Iterable<EntityPlayerMP> players) {
    this.action = actionIn;
  ...
  public void readPacketData(PacketBuffer buf) throws IOException {
    this.action =
        (S38PacketPlayerListItem.Action) buf.readEnumValue(S38PacketPlayerListItem.Action.class);
    ...
    for (int j = 0; j < i; ++j) {
        ...
        case ADD_PLAYER:
          ...
          for (; i1 < l; ++i1) {
            ...
            if (buf.readBoolean()) {
              gameprofile
                  .getProperties()
                  .put(s, new Property(s, s1, buf.readStringFromBuffer(32767)));
            } else {
      ...
      this.players.add(
          new S38PacketPlayerListItem.AddPlayerData(
              gameprofile, k, worldsettings$gametype, ichatcomponent));
    }
  }
      ...
      switch (this.action) {
        case ADD_PLAYER:
          ...
          for (Property property :
              s38packetplayerlistitem$addplayerdata.getProfile().getProperties().values()) {
