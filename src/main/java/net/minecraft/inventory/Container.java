...
public abstract class Container {
   ...
   * Handles slot click.
   *
   * @param mode 0 = basic click, 1 = shift click, 2 = hotbar, 3 = pick block, 4 = drop, 5 = ?, 6 =
   *     double click
   */
    ...
    if (mode == 5) {
      ...
      if (slotId == -999) {
        if (inventoryplayer.getItemStack() != null) {
          ...
          if (clickedButton == 1) {
            playerIn.dropPlayerItemWithRandomChoice(
      ...
      if (slot5.canTakeStack(playerIn)) {
        ...
        if (slot5.getHasStack() && flag) {
          ...
          if ((slot5.inventory != inventoryplayer || !slot5.isItemValid(itemstack7))
              && itemstack7 != null) {
            if (k1 > -1) {
      ...
      if (slot3 != null && slot3.getHasStack() && slot3.canTakeStack(playerIn)) {
        ItemStack itemstack5 =
            slot3.decrStackSize(clickedButton == 0 ? 1 : slot3.getStack().stackSize);
      ...
      if (itemstack4 != null
          && (slot2 == null || !slot2.getHasStack() || !slot2.canTakeStack(playerIn))) {
        ...
        for (int l2 = 0; l2 < 2; ++l2) {
          for (int i3 = i1;
            ...
            if (slot8.getHasStack()
              ...
              int l =
                  Math.min(
                      itemstack4.getMaxStackSize() - itemstack4.stackSize,
  ...
  /**
   * Called to determine if the current slot is valid for the stack merging (double-click) code. The
   * stack passed in is null for the initial slot that was double-clicked.
   */
  ...
  protected void retrySlotClick(
      int slotId, int clickedButton, boolean mode, EntityPlayer playerIn) {
  ...
  /**
   * Merges provided ItemStack with the first avaliable one in the container/player inventor between
   * minIndex (included) and maxIndex (excluded). Args : stack, minIndex, maxIndex,
   * negativDirection. /!\ the Container implementation do not check if the item is valid for the
   * slot
   */
  ...
  /**
   * Extracts the drag mode. Args : eventButton. Return (0 : evenly split, 1 : one item by slot, 2 :
   * not used ?)
   */
  ...
  public static boolean canAddItemToSlot(Slot slotIn, ItemStack stack, boolean stackSizeMatters) {
    ...
    if (slotIn != null
      ...
      flag |=
          slotIn.getStack().stackSize + (stackSizeMatters ? 0 : stack.stackSize)
  ...
  /**
   * Compute the new stack size, Returns the stack with the new size. Args : dragSlots, dragMode,
   * dragStack, slotStackSize
   */
  public static void computeStackSize(
      Set<Slot> p_94525_0_, int p_94525_1_, ItemStack p_94525_2_, int p_94525_3_) {
    switch (p_94525_1_) {
      case 0:
        p_94525_2_.stackSize =
            MathHelper.floor_float((float) p_94525_2_.stackSize / (float) p_94525_0_.size());
        break;
  ...
  /**
   * Like the version that takes an inventory. If the given TileEntity is not an Inventory, 0 is
   * returned instead.
   */
