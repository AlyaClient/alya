...
public class TextureAnimation {
  ...
  public TextureAnimation(
    ...
    if (srcData.length % i != 0) {
      Config.warn(
          "Invalid animated texture length: "
              + srcData.length
              + ", frameWidth: "
              + frameWidth
              + ", frameHeight: "
  ...
  public void updateTexture() {
    if (this.valid) {
      ...
      this.active =
          SmartAnimations.isActive() ? SmartAnimations.isTextureRendered(this.dstTextId) : true;
            ...
            if (i + j <= this.imageData.limit()) {
              if (this.interpolate && textureanimationframe.counter > 0) {
                if (this.interpolateSkip <= 1
                  ...
                  TextureAnimationFrame textureanimationframe1 =
                      this.getFrame(this.currentFrameIndex + 1);
                  double d0 =
                      1.0D
                          * (double) textureanimationframe.counter
  ...
  private void updateTextureInerpolate(
      TextureAnimationFrame frame1, TextureAnimationFrame frame2, double dd) {
    ...
    if (j + i <= this.imageData.limit()) {
      ...
      if (k + i <= this.imageData.limit()) {
        ...
        GL11.glTexSubImage2D(
            ...
            GL11.GL_RGBA,
            GL11.GL_UNSIGNED_BYTE,
            (ByteBuffer) this.interpolateData);
      }
    }
  }
