    ...
    public static IBakedModel getLeavesModel(IBakedModel model, IBlockState stateIn) {
        if(!Config.isTreesSmart()) {
            ...
            return list == generalQuadsCullAcacia
                    ...
                    : (list == generalQuadsCullOak
                    ? modelLeavesDoubleOak
                    : (list == generalQuadsCullSpruce ? modelLeavesDoubleSpruce : model)))));
        }
    }
    ...
    public static boolean isSameLeaves(IBlockState state1, IBlockState state2) {
        if(state1 == state2) {
            ...
            return block != block1
                    ...
                    ? ((BlockPlanks.EnumType) state1.getValue(BlockNewLeaf.VARIANT))
                    .equals(state2.getValue(BlockNewLeaf.VARIANT))
                    : false));
        }
    }
    ...
    private static IBakedModel getModelDoubleFace(IBakedModel model) {
        if(model == null) {
            ...
            for(int i = 0; i < aenumfacing.length; ++i) {
                ...
                if(list.size() != 1) {
                    Config.warn(
                            "SmartLeaves: Model is not cube, side: "
                                    + enumfacing
                                    + ", quads: "
                                    + list.size()
                                    + ", model: "
            ...
            for(int k = 0; k < aenumfacing.length; ++k) {
                ...
                BakedQuad bakedquad1 =
                        new BakedQuad(
                                (int[]) bakedquad.getVertexData().clone(),
