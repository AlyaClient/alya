...
public class BlockModelCustomizer {
    ...
    public static IBakedModel getRenderModel(
            IBakedModel modelIn, IBlockState stateIn, RenderEnv renderEnv) {
    ...
    public static List<BakedQuad> getRenderQuads(
        ...
        for(int i = 0; i < quads.size(); ++i) {
            ...
            BakedQuad[] abakedquad =
                    getRenderQuads(bakedquad, worldIn, stateIn, posIn, enumfacing, rand, renderEnv);

            if(i == 0
                    && quads.size() == 1
                    && abakedquad.length == 1
                    && abakedquad[0] == bakedquad
            ...
            for(int j = 0; j < abakedquad.length; ++j) {
                ...
                if(bakedquad1.getQuadEmissive() != null) {
                    renderEnv
                            .getListQuadsOverlay(getEmissiveLayer(layer))
                            .addQuad(bakedquad1.getQuadEmissive(), stateIn);
    ...
    private static EnumWorldBlockLayer getEmissiveLayer(EnumWorldBlockLayer layer) {
        return layer != null && layer != EnumWorldBlockLayer.SOLID
                ? layer
    ...
    private static BakedQuad[] getRenderQuads(
        ...
        if(renderEnv.isBreakingAnimation(quad)) {
            ...
            if(Config.isConnectedTextures()) {
                BakedQuad[] abakedquad =
                        ConnectedTextures.getConnectedTexture(worldIn, stateIn, posIn, quad, renderEnv);
