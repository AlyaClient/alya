...
public class CustomEntityModelParser {
    ...
    public static CustomEntityRenderer parseEntityRender(JsonObject obj, String path) {
        ...
        CustomModelRenderer[] acustommodelrenderer =
                (CustomModelRenderer[])
                        ((CustomModelRenderer[]) list.toArray(new CustomModelRenderer[list.size()]));
        ...
        CustomEntityRenderer customentityrenderer =
                new CustomEntityRenderer(s, s1, resourcelocation, acustommodelrenderer, f);
    ...
    private static void processExternalModel(JsonObject elem, Map mapModelJsons, String basePath) {
        ...
        if(s != null) {
            ...
            try {
                ...
                Config.error(
                        "" + jsonparseexception.getClass().getName() + ": " + jsonparseexception.getMessage());
    ...
    public static CustomModelRenderer parseCustomModelRenderer(
            JsonObject elem, int[] textureSize, String basePath) {
        ...
        if(jsonarray != null) {
            ...
            if(list.size() > 0) {
                ModelVariableUpdater[] amodelvariableupdater =
                        (ModelVariableUpdater[])
                                ((ModelVariableUpdater[]) list.toArray(new ModelVariableUpdater[list.size()]));
                modelupdater = new ModelUpdater(amodelvariableupdater);
            }
        }

        ModelRenderer modelrenderer =
                PlayerItemParser.parseModelRenderer(elem, modelbase, textureSize, basePath);
        CustomModelRenderer custommodelrenderer =
                new CustomModelRenderer(s, flag, modelrenderer, modelupdater);
