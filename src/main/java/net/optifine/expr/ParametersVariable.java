...
public class ParametersVariable implements IParameters {
    ...
    public ParametersVariable(
            ExpressionType[] first, ExpressionType[] repeat, ExpressionType[] last) {
        this(first, repeat, last, Integer.MAX_VALUE);
    }

    public ParametersVariable(
            ExpressionType[] first, ExpressionType[] repeat, ExpressionType[] last, int maxCount) {
    ...
    public ExpressionType[] getParameterTypes(IExpression[] arguments) {
        ...
        for(int l = 0;
            l + this.repeat.length <= j && i + l + this.repeat.length <= this.maxCount;
        ...
        ExpressionType[] aexpressiontype =
                (ExpressionType[]) list.toArray(new ExpressionType[list.size()]);
