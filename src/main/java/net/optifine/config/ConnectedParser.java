...
public class ConnectedParser {
            ...
            if(block == Blocks.double_plant && md > 7) {
                ...
                iblockstate =
                        iblockstate.withProperty(
                                BlockDoublePlant.VARIANT, iblockstate1.getValue(BlockDoublePlant.VARIANT));
            }
    ...
    public BiomeGenBase[] parseBiomes(String str) {
        if(str == null) {
            ...
            BiomeGenBase[] abiomegenbase =
                    (BiomeGenBase[]) ((BiomeGenBase[]) list.toArray(new BiomeGenBase[list.size()]));
    ...
    public <T> T[] parseObjects(
            String str, T[] objs, INameGetter nameGetter, String property, T[] errValue) {
        if(str == null) {
            return null;
        } else {
            str = str.toLowerCase().trim();
    ...
    public EnumDyeColor[] parseDyeColors(String str, String property, EnumDyeColor[] errValue) {
        return (EnumDyeColor[])
                this.parseObjects(str, EnumDyeColor.values(), NAME_GETTER_DYE_COLOR, property, errValue);
    }

    public Weather[] parseWeather(String str, String property, Weather[] errValue) {
        return (Weather[])
                this.parseObjects(str, Weather.values(), NAME_GETTER_ENUM, property, errValue);
    }
    ...
    public VillagerProfession[] parseProfessions(String profStr) {
        if(profStr == null) {
            ...
            if(list.isEmpty()) {
                ...
                VillagerProfession[] avillagerprofession =
                        (VillagerProfession[])
                                ((VillagerProfession[]) list.toArray(new VillagerProfession[list.size()]));
