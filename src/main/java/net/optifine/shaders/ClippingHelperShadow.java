...
public class ClippingHelperShadow extends ClippingHelper {
  ...
  public boolean isBoxInFrustum(double x1, double y1, double z1, double x2, double y2, double z2) {
    for (int i = 0; i < this.shadowClipPlaneCount; ++i) {
      ...
      if (this.dot4(afloat, x1, y1, z1) <= 0.0D
          && this.dot4(afloat, x2, y1, z1) <= 0.0D
          && this.dot4(afloat, x1, y2, z1) <= 0.0D
          && this.dot4(afloat, x2, y2, z1) <= 0.0D
          && this.dot4(afloat, x1, y1, z2) <= 0.0D
          && this.dot4(afloat, x2, y1, z2) <= 0.0D
          && this.dot4(afloat, x1, y2, z2) <= 0.0D
          && this.dot4(afloat, x2, y2, z2) <= 0.0D) {
        return false;
      }
    }
  ...
  private double dot3(float[] vecA, float[] vecB) {
    return (double) vecA[0] * (double) vecB[0]
        + (double) vecA[1] * (double) vecB[1]
  ...
  private void normalizePlane(float[] plane) {
    float f =
        MathHelper.sqrt_float(plane[0] * plane[0] + plane[1] * plane[1] + plane[2] * plane[2]);
    plane[0] /= f;
    plane[1] /= f;
    plane[2] /= f;
    plane[3] /= f;
  ...
  private void makeShadowPlane(
      float[] shadowPlane, float[] positivePlane, float[] negativePlane, float[] vecSun) {
  ...
  public void init() {
    ...
    if (f >= 0.0F) {
      ...
      if (f > 0.0F) {
        if (f2 < 0.0F) {
          this.makeShadowPlane(
              this.shadowClipPlanes[this.shadowClipPlaneCount++],
              this.frustum[0],
              this.frustum[2],
              afloat3);
        }
