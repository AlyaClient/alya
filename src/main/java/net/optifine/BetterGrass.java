...
public class BetterGrass {
    ...
    if (spritesLoaded) {
      ...
      if (grassMultilayer) {
        IBakedModel ibakedmodel =
            BlockModelUtils.makeModelCube((TextureAtlasSprite) spriteGrassSide, -1);
  ...
  private static void loadProperties(TextureMap textureMap) {
    ...
    try {
      ...
      spriteGrassSide =
          registerSprite(properties, "texture.grass_side", "blocks/grass_side", textureMap);
      spriteMycelium =
          registerSprite(properties, "texture.mycelium", "blocks/mycelium_top", textureMap);
      spritePodzol =
          registerSprite(properties, "texture.podzol", "blocks/dirt_podzol_top", textureMap);
  ...
  private static TextureAtlasSprite registerSprite(
      Properties props, String key, String textureDefault, TextureMap textureMap) {
    ...
    if (Config.isBetterGrassFancy()) {
      if (flag) {
        if (betterMyceliumSnow && getBlockAt(blockPos, facing, blockAccess) == Blocks.snow_layer) {
          ...
          && getBlockAt(blockPos.down(), facing, blockAccess) == Blocks.mycelium) {
