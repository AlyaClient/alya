...
public class ConnectedProperties {
    ...
    public ConnectedProperties(Properties props, String path) {
        ...
        this.tintBlockState =
                connectedparser.parseBlockState(
                        props.getProperty("tintBlock"), Blocks.air.getDefaultState());
        this.layer =
                connectedparser.parseBlockRenderLayer(
                        props.getProperty("layer"), EnumWorldBlockLayer.CUTOUT_MIPPED);
    }
    ...
    private String[] parseTileNames(String str) {
        if(str == null) {
            ...
            for(int i1 = 0; i1 < astring2.length; ++i1) {
                ...
                if(!s1.startsWith(this.basePath)
                        && !s1.startsWith("textures/")
                        && !s1.startsWith("mcpatcher/")) {
                    s1 = this.basePath + "/" + s1;
                }
    ...
    private static TextureAtlasSprite[] registerIcons(
            String[] tileNames, TextureMap textureMap, boolean skipTiles, boolean defaultTiles) {
        if(tileNames == null) {
            return null;
        } else {
            List list = new ArrayList();
            ...
            TextureAtlasSprite[] atextureatlassprite =
                    (TextureAtlasSprite[])
                            ((TextureAtlasSprite[]) list.toArray(new TextureAtlasSprite[list.size()]));
    ...
    public boolean matchesBlock(int blockId, int metadata) {
        return !Matches.block(blockId, metadata, this.matchBlocks)
                ? false
                : Matches.metadata(metadata, this.metadatas);
    }
