...
public final class MicrosoftAuth {
    ...
    public static CompletableFuture<String> acquireMSAuthCode(
            final Consumer<URI> browserAction, final Executor executor) {
        return CompletableFuture.supplyAsync(
                () -> {
                    try {
                        ...
                        server.createContext(
                                ...
                                exchange -> {
                                    ...
                                    if(!state.equals(queryParams.get("state"))) {
                                        String em =
                                                String.format(
                                                        "State mismatch expected %s got %s", state, queryParams.get("state"));
                                        errorMsg.set(em);
                        ...
                        URI uri =
                                new URIBuilder("https://login.live.com/oauth20_authorize.srf")
                                        .addParameter("client_id", CLIENT_ID)
                                        .addParameter("response_type", "code")
                                        .addParameter(
                                                "redirect_uri",
                                                String.format(
                                                        "http://localhost:%d/callback", server.getAddress().getPort()))
    ...
    public static CompletableFuture<String> acquireMSAccessToken(
            final String authCode, final Executor executor) {
        return CompletableFuture.supplyAsync(
                () -> {
    ...
    public static CompletableFuture<String> acquireMCAccessToken(
        ...
        return CompletableFuture.supplyAsync(
                () -> {
                    try(CloseableHttpClient client = HttpClients.createMinimal()) {
                        HttpPost request =
                                new HttpPost("https://api.minecraftservices.com/authentication/login_with_xbox");
                        request.setConfig(REQUEST_CONFIG);
                        request.setHeader("Content-Type", "application/json");
                        request.setEntity(
                                new StringEntity(
        ...
        return CompletableFuture.supplyAsync(
                () -> {
                    try(CloseableHttpClient client = HttpClients.createMinimal()) {
                        ...
                        Session session =
                                new Session(
                                        json.get("name").getAsString(), uuid, mcToken, Session.Type.MOJANG.toString());
                        Alya.getInstance()
                                .getLogger()
                                .debug("login successful for {}", json.get("name").getAsString());
                        return session;
                    } catch(Exception e) {
