...
public final class ConfigManager {
    ...
    public ConfigManager() {
        ...
        if(!configDir.exists()) {
            if(!configDir.mkdirs()) {
                Alya.getInstance().getLogger().error("Failed to create config directory: {}", configDir.getAbsolutePath());
            }
        }
    }
        ...
        try(FileReader reader = new FileReader(configFile)) {
            ...
            if(root.has("modules")) {
                ...
                for(final Map.Entry<String, JsonElement> entry : modules.entrySet()) {
                    ...
                    Alya.getInstance().getModuleManager().getModule(moduleName).ifPresent(module -> {
                        ...
                        if(moduleData.has("settings")) {
                            ...
                            for(final Map.Entry<String, JsonElement> settingEntry : settingsData.entrySet()) {
                                module.getSetting(settingEntry.getKey()).ifPresent(setting -> setting.setValueFromString(settingEntry.getValue().getAsString()));
                            }
                        }
                    });
                }
