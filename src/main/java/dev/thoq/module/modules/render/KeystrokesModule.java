...
public final class KeystrokesModule extends Module {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        drawKey(fontRenderer, forwardX, startY, KEY_SIZE, getKeyName(settings.keyBindForward), !isChatOpen && isKeyPressed(settings.keyBindForward));
        final int row2Y = startY + KEY_SIZE + KEY_SPACING;
        drawKey(fontRenderer, startX, row2Y, KEY_SIZE, getKeyName(settings.keyBindLeft), !isChatOpen && isKeyPressed(settings.keyBindLeft));
        drawKey(fontRenderer, startX + KEY_SIZE + KEY_SPACING, row2Y, KEY_SIZE, getKeyName(settings.keyBindBack), !isChatOpen && isKeyPressed(settings.keyBindBack));
        drawKey(fontRenderer, startX + (KEY_SIZE + KEY_SPACING) * 2, row2Y, KEY_SIZE, getKeyName(settings.keyBindRight), !isChatOpen && isKeyPressed(settings.keyBindRight));
        ...
        if(showSpace.isEnabled()) {
            drawKey(fontRenderer, startX, currentY, SPACE_WIDTH, "SPACE", !isChatOpen && isKeyPressed(settings.keyBindJump));
        }
    }
